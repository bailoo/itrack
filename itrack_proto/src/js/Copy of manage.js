  ...
  function show_manage_interfaces(type, option)   // option = add_feature
	...
	function manage_select_by_entity(options)
	{
  ...
  function action_manage_account(action_type)
   ...
   {
      var result = false;   
      result = validate_manage_add_device(action_type); 
   ...
   function action_manage_device_sale(obj, action_type)
   {
	...
	function action_manage_vehicle(obj, action_type)
	{
		...
		else if(action_type=="edit" || action_type=="delete")
		{
			...
			if(result != false)
			{
				if(action_type=="edit")
				{
					var file_name="src/php/manage_edit_vehicle_1.php";
				}
				else if(action_type=="delete")
				{
    ...
    function action_manage_group(obj, action_type)
   {  
      var res = false;
      
      if(action_type == "add")
        res = validate_manage_add_group(obj);
      else if(action_type =="edit")
        res = validate_manage_edit_group(obj); 
      else if(action_type == "delete")
        res = true;            
 ...
 {  
    if(action_type=="add")  
    {
		var obj=document.manage1.elements['manage_user[]'];
