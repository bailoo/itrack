<?php	
	...
	$query="SELECT * FROM schedule_assignment_upl WHERE vehicle_id='$vehicleid_to_location' AND status='1'";
	//echo "query=".$query."<br>";
	...
	if($row_result>0)
	{
		...
		while($row=mysql_fetch_object($result))
		{
			echo"<tr>
					...
					if($row->location_id!="")
					{
						$tmp_location_id=$row->location_id;
						//echo "l=".$tmp_location_id."<br>";
						$location_id_a=explode(":",$tmp_location_id);
						$lqc=""; //lqc=location query condition
						foreach($location_id_a as $la1)
						{
							$la_arr=explode(",",$la1);
							foreach($la_arr as $la)
							{
								$lqc=$lqc."location_id=".$la." OR ";
							}
							
						}
						/*for($i=0;$i<sizeof($location_id_a);$i++)
						{
							$lqc=$lqc."location_id=".$location_id_a[$i]." OR ";
						}*/				
						$lqc=substr($lqc,0,-3);
						//echo "SELECT * FROM schedule_location WHERE $lqc"."<br>";
						$Query_Location="SELECT * FROM schedule_location_upl WHERE $lqc";					
					...
					*/
					echo"<td>
							<input type='button' Value='Delete' onclick=javascript:manage_delete_schedule_assignment_upl(".$row->serial.",".$vehicleid_to_location.",'".$vehicle_name."')>
						</td>
						<!--<td>
							<input type='button' Value='Edit' onclick=javascript:manage_edit_schedule_assignment_upl(".$row->serial.",".$vehicleid_to_location.",'".$vehicle_name."','".$row->date_to."')>
						</td>-->
				</tr>";
			$cnt++;
		}
			...
			<div id="divpopup_edit_sa">
						<table cellspacing="5" cellpadding="5" border=0>
							<tr>
								...
								<td>
									<input type="button" Value="Edit Date" onclick="javascript:manage_edit_schedule_assignment_1_upl()">
								</td>
								<td>
