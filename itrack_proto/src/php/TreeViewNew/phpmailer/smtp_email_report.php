...
$datetime_sent2 =date("Y-m-d H:i:s");	

$query = "SELECT * FROM mail_info_report WHERE status=1";
//echo "\nquery=".$query;
...
while($row = mysql_fetch_object($result))
{
  ...
  */
  ...
  $email_from1[] = "support@iembsys.com";
  ...
  $query = "UPDATE mail_info_report SET datetime_sent='$datetime_sent2',status=0 WHERE sno='$sno'";
...
for($i=0;$i<sizeof($email_to1);$i++)
{
  ...
  echo "\nemail_to=".$email_to." ,email_subject=".$email_subject." ,email_message=".$email_message." ,email_from=".$email_from."\n";
