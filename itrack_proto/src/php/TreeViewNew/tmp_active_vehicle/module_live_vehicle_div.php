		...
		<table width="100%" cellspacing=0 cellpadding=0>
			...
			<tr valign="top">
				<td align="center">
					...
					if($flag_station)
					{
						echo '<div id="show_route_ev" style="height:170px;overflow:auto;display:none;">						
							<font color="#034A24">';
								echo '<table>';
								
								$query_assigned = "SELECT DISTINCT vehicle_assignment.device_imei_no,route_assignment2.route_name_ev FROM vehicle_assignment,route_assignment2,vehicle WHERE route_assignment2.user_account_id='$account_id' AND vehicle.vehicle_name=route_assignment2.vehicle_name AND vehicle.vehicle_id = vehicle_assignment.vehicle_id AND route_assignment2.status=1 AND NOT(route_assignment2.route_name_ev='') ORDER BY route_assignment2.route_name_ev ASC";
								//echo $query_assigned;
								...
								while($row=mysql_fetch_object($result_assigned))
								{
									...
									echo '<tr>
										<td style="font-size:10px;"><input type="checkbox" name="route_ev[]" value="'.$route_str.'"/>&nbsp;'.$route.'<td>';																		
									echo'</tr>';									
								}		
								echo'</table>
							</font>
						...
						echo '<div id="show_route_mor" style="height:170px;overflow:auto;display:none;">						
							<font color="#034A24">';
								...
								$query_assigned = "SELECT DISTINCT vehicle_assignment.device_imei_no,route_assignment2.route_name_mor FROM vehicle_assignment,route_assignment2,vehicle WHERE route_assignment2.user_account_id='$account_id' AND vehicle.vehicle_name=route_assignment2.vehicle_name AND vehicle.vehicle_id = vehicle_assignment.vehicle_id AND route_assignment2.status=1 AND NOT(route_assignment2.route_name_mor ='') ORDER BY route_assignment2.route_name_mor ASC";
								//echo $query_assigned;
								/*$query_assigned = "SELECT vehicle_assignment.device_imei_no route_assignment2.route_name ".
								 "FROM vehicle_assignment,route_assignment2 WHERE route_assignment2.user_account_id='$account_id' AND ".
								 "route_assignment2.status=1 ORDER BY route_assignment2.route_name ASC";*/
								...
								while($row=mysql_fetch_object($result_assigned))
								{
									...
									echo '<tr>
										<td style="font-size:10px;"><input type="checkbox" name="route_mor[]" value="'.$route_str.'"/>&nbsp;'.$route.'<td>
									</tr>';								
								}		
								echo'</table>
							</font>
			...
			<TR>			
				<TD align="center">
					<input type="button" Onclick="javascript:filter_live_vehicle(this.form)" value="Enter"> &nbsp;
