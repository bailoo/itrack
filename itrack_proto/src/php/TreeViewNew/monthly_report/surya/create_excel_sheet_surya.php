<?php
  ...
  $imei_param = $vserial_global[$j];
  
  $worksheet =& $workbook->addworksheet($imei_param);  
  $report_title = "SURYA PHARMA- MONTHLY VTS REPORT -Vehicle:".$vname_global[$j]." Month-($previous_month)";
  ...
  $worksheet->write($r, 5, "Travel StartTime", $text_format);
  $worksheet->set_column(5, 5, 22); 
  $worksheet->write($r, 6, "Travel EndTime", $text_format);
  ...
  for($q=0;$q<$total_days;$q++)                       //******** TOTAL DAYS IN PREVIOUS MONTH
  {
    ...
    $daily_travel_startime_csv = $daily_travel_startime[$j][$q]; 
    $daily_travel_endtime_csv = $daily_travel_endtime[$j][$q]; 
      
    //NO OF COLUMNS =4 (SNO, STARTITME, ENDTIME, DISTANCE, HALT)          
    echo "\nstarttime=".$starttime." ,endtime=".$endtime." ,distance_csv=".$distance_csv." ,halt_csv=".$halt_csv." travel_starttime=".$daily_travel_startime_csv." ,travel_endtime=".$daily_travel_endtime_csv;
    //echo "\nbefore write";
    ...
    $worksheet->write($r,5, $daily_travel_startime_csv);
    $worksheet->write($r,6, $daily_travel_endtime_csv);
    //echo "\nafter write";
