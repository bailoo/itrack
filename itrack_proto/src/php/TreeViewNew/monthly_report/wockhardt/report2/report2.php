...
/////********* CREATE EXCEL FILE *******************///////
$filename_title = 'MONTHLY_WOCKHARDT_VTS_REPORT_2_'.$prev_date_title;
//$file_path = "/var/www/html/vts/beta/src/php/download/".$filename_title;
//ONLINE
$file_path = "/var/www/html/vts/beta/src/php/monthly_report/wockhardt/report2/excel_data/".$filename_title;
...
$query = "SELECT vehicle_assignment.device_imei_no, schedule_assignment.vehicle_id, schedule_assignment.min_operation_time, ".
        ...
        "schedule_assignment.min_halt_time, schedule_assignment.max_halt_time, schedule_assignment.nonpoi_halt_time,".
      ...
      //{
        ...
        for($y=0;$y<=($date_size-1);$y++)                       //CHECKS EVERYTIME FOR ONE RECORD AND BREAKS- OUTER LOOP
          ...
          for($z=0;$z<sizeof($day_db);$z++)                     //CHECKS EVERYTIME FOR ONE RECORD AND BREAKS- INNER LOOP
          {
            ...
            {                            
              //echo "\nDAY SPECIFIED";
              add_data_to_field($imei[$x], $vname[$x], $date1, $date2, $op_date1, $op_date2, $min_halt_time, $max_halt_time, $nonpoi_halt_time[$x], $geo_point1, $border1, $border2, $worksheet);          
              $wflag = 1;
              break;
            }
          }       	 
...
{      
    ...
    //$worksheet->set_column($c, $c, 8);
    $c++;
    $worksheet->write($r,$c, round($non_poi_string_final,2), $border2);       //NON POI HALT
    //$worksheet->set_column($c, $c, 13);            
}    
?>
