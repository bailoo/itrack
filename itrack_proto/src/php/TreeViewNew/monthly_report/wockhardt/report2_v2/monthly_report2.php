...
include_once("get_monthly_data.php");
...
/////********* CREATE EXCEL FILE *******************///////
$rno = rand();
$filename_title = 'MONTHLY_WOCKHARDT_VTS_REPORT_2_('.$date.")_".$rno;  
...
$weekday1 = date("w",strtotime($date))+1;     //actually return 0 =SUN, 6=SAT  now convert to => 1=sun, 7=sat
...
$query3 = "SELECT DISTINCT vehicle_assignment.device_imei_no, schedule_assignment.vehicle_id, schedule_assignment.min_operation_time, ".
        "schedule_assignment.max_operation_time,schedule_assignment.min_halt_time,schedule_assignment.max_halt_time,schedule_assignment.nonpoi_halt_time, schedule_assignment.date_from, schedule_assignment.date_to,".
...
while($row3 = mysql_fetch_object($result3))
{
	...
	if($by_day_tmp == 1)
	{
		//echo "\nIF:size=".sizeof($day_db1)." ,date=".$date;		
	...
	{		
		...
		if($row4 = mysql_fetch_object($result4))
		{
		...
		{    
		  ...
		  //{
			...
			{           
			  ...
			  $weekday = date("w",$timestmp)+1;     //1 =SUN, 7=SAT
			  //echo "\nWEEKDAY=".$weekday;
