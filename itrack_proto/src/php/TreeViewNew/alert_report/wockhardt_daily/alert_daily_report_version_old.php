...
{  	
  ...
  //echo $query2."\n";
    ...
    if($valid_day)
    {
      ...
      get_daily_report($vserial[$i], $vname, $date1, $date2, $min_operation_time ,$max_operation_time ,$min_halt_time, $max_halt_time, $min_distance_travelled, $max_distance_travelled, $geo_point);
    }  // IF VALID DAY CLOSED
  } //WHILE CLOSED    
} // FOR CLOSED
...
function get_daily_report($imei, $vname, $startdate, $enddate, $min_operation_time ,$max_operation_time ,$min_halt_time, $max_halt_time, $min_distance_travelled, $max_distance_travelled, $geo_point)
{
...
function send_email()
{
  ...
  for($i=0;$i<sizeof($alert_imei_dist);$i++)         //DISTANCE ALERT
  {       
    $alert_msg = $alert_vname_dist[$i]." moved ".$alert_distance[$i]." km\n";
    $alert_string = $alert_string.$alert_msg."\n";    
  }
  ...
  {   
    ...
    $alert_msg = $alert_vname_halt[$i]." halt time is ".$alert_halt_time_formated."\n";
    $alert_string = $alert_string.$alert_msg."\n";    
  }  
  
  //$alert_string = "vehicle total dist is 50 km";
