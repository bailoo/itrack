    ...
    if($valid_day)
    {
      ...
      for($z=0;$z<sizeof($intermediate_time_tmp);$z++)
      {
        ...
        {          
          ...
          get_alert_intermediate_data($vehicle_id, $vserial[$i], $vname, $landmark_coord);
          break;
        }           
      }
    }  // IF VALID DAY CLOSED
...
function get_alert_intermediate_data($vehicle_id, $imei, $vname, $landmark_coord)
{
  global $alert_imei;
  global $alert_vname;
  global $alert_msg;
...
function send_email()
{
  ...
  for($i=0;$i<sizeof($alert_imei);$i++)
  {
    ...
    $query = "SELECT DISTINCT person_email FROM escalation WHERE escalation_id IN (SELECT escalation_id FROM alert_assignment WHERE ".
              "mail_status=1 AND vehicle_id='$alert_vid[$i]' AND status=1) AND approved_status=1";
    $result = mysql_query($query);
