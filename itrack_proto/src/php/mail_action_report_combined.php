...
function write_report($vserial, $vid, $vname, $user_interval)
{
  ...
  {  	            
    ...
    $overall_dist = 0.0;
    $sno_dist = 1;
    //$title = $vname[$i]." (".$vserial[$i]."), DateTime , ".$startdate.",".$enddate;
...
$filename_title = 'VTS_REPORT_'.$date1.'-'.$date2;
//$filename_title = 'VTS_REPORT';
...
{      
	...
	while($row = mysql_fetch_object($result_vehicle))      // LOOP FOR MULTIPLE ALERTS
	{
      ...
      {	
		for($k=0;$k<$h;$k++)
		{
		//echo "db vehicle id=".$vid_db[$k]."vehicle_id=".$vid[$j]."<br>";
        if($vid[$j] == $vid_db[$k])
        {
          ...
          $query_last_datetime = "SELECT last_datetime FROM report_last_mail_status WHERE vehicle_id ='$vid_db[$k]' AND ".
                                  "alert_id='$alert_db[$k]' AND escalation_id='$escalation_id[$i]'  AND report_type='distance'";
          //echo "\n".$query_last_datetime;
			...
			//echo "in else<br>";
             ...
             else
             {
              ...
              $query_update2 = "insert into report_last_mail_status(last_datetime,vehicle_id,alert_id,report_type,escalation_id)".
                              "VALUES('$last_datetime','$vid_db[$k]','$alert_db[$k]','distance','$escalation_id[$i]')";
    ...
    {    
        ...
        $r=0;   //row
		echo "escalation_id=".$escalation_id[$i]."<br><br>csv_string_dist_final".$csv_string_dist_final."<br><br>";
        ...
        { 
		  ...
		  if($p==0)
		  {
			...
			// echo "\csv_string_dist_final=".$csv_string_dist_final."<br><br>";
		  }
          ...
          for($q=0;$q<sizeof($sheet1_row);$q++)
          {
            ...
            {
              if( ($m ==0) && ($sheet1_data[$m]!="") ) //SERIAL
              {
                $worksheet1->write($r,$m, $sno_tmp);
              }
              else if($q == (sizeof($sheet1_row)-1) && ($m==0) )
              {
                //echo "\nBLANK DISTANCE";
                $worksheet1->write_blank($r, $m,                 $blank_format);
              }
              else
              {
          ...
          $worksheet1->write_blank($r, 1,                 $blank_format);
          $worksheet1->write_blank($r, 2,                 $blank_format);
          $worksheet1->write_blank($r, 3,                 $blank_format);
          $worksheet1->write_blank($r, 4,                 $blank_format);          
          $r++;         
        }     
        ...
        for($p=0;$p<sizeof($sheet2);$p++)
        {
		 ...
		 /////////// for debugging purpose
		  if($p==0)
		  {
			 ...
			 //echo "\csv_string_halt_final=".$csv_string_halt_final."<br><br>";
		  }
          ...
          {       
            ...
            {           
              if( ($m ==0) && ($sheet2_data[$m]!="") ) //SERIAL
              {
                $worksheet2->write($r,$m, $sno_tmp);
              }
              else if($q == (sizeof($sheet2_row)-1) && ($m==0) && ($sheet2_data[$m]!="") )
              {
                $worksheet2->write_blank($r, $m,                 $blank_format);
              }              
              else
              {              
          ...
          $worksheet2->write_blank($r, 1,                 $blank_format);
          $worksheet2->write_blank($r, 2,                 $blank_format);
          $worksheet2->write_blank($r, 3,                 $blank_format);
          $worksheet2->write_blank($r, 4,                 $blank_format);
          $worksheet2->write_blank($r, 5,                 $blank_format);           
        }
        ...
        {        
		  ...
		  if($p==0)
		  {
			...
			// echo "\csv_string_travel_final=".$csv_string_travel_final."<br><br>";
		  }
          ...
          {      
            ...
            {   
              if( ($m ==0) && ($sheet3_data[$m]!="") ) //SERIAL
              {
                $worksheet3->write($r,$m, $sno_tmp);
              }
              else if($q == (sizeof($sheet3_row)-1) && ($m==0) && ($sheet3_data[$m]!=""))
              {
                $worksheet3->write_blank($r, $m,                 $blank_format);
              }              
              else
              {              
          ...
          $worksheet3->write_blank($r, 1,                 $blank_format);
          $worksheet3->write_blank($r, 2,                 $blank_format);
          $worksheet3->write_blank($r, 3,                 $blank_format);
          $worksheet3->write_blank($r, 4,                 $blank_format);
          $worksheet3->write_blank($r, 5,                 $blank_format);          
          $worksheet3->write_blank($r, 6,                 $blank_format);
          $worksheet3->write_blank($r, 7,                 $blank_format);
          $r++;  
        }                           
      //******************* CLOSE EXCEL FILE************************************////                             
