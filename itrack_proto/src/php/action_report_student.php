...
$student_str = $_POST['studentserials'];
//echo "<br>student_str=".$student_str; 
$school_id = $_POST['school_id'];
//echo "<br>devicestr=".$device_str;
//echo "<br>school_id=".$school_id; 
$student_ids = explode(':',$student_str);
$ssize=count($student_ids);
$j=0;
for($i=0;$i<sizeof($student_ids);$i++)
{
  $query="SELECT bus_id_pick,bus_id_drop,studentcard_number,student_name from student,studentcard where student_id='$student_ids[$i]' AND student.studentcard_id=studentcard.studentcard_id AND studentcard.status='1' AND student.status='1'";
	//	echo "query=".$query."<br>";
	...
	if($row_result!=null)
	{
		//$i=0;
				...
				{									
					$bus_id_pick= $row->bus_id_pick;
					$bus_id_drop= $row->bus_id_drop;
					$studentcard_number= $row->studentcard_number;
					$student_name= $row->student_name;
          //
          if($bus_id_pick==$bus_id_drop)
          {
              if($j==0)
              {
                $bus_id_string= $bus_id_pick;
                $final_bus_ids[]= $bus_id_pick;
                $final_student_ids[]= $student_ids[$i];
                $card_number[]=$studentcard_number;
                $student_names[]=$student_name;
              }
              else
              {
                 $bus_id_string=$bus_id_string.",".$bus_id_pick;
                 $final_bus_ids[]= $bus_id_pick;
                 $final_student_ids[]= $student_ids[$i];
                 $card_number[]=$studentcard_number;
                 $student_names[]=$student_name;	
          ...
          else
          {
                if($j==0)
                {
                  ...
                  $bus_id_string=$bus_id_string.",".$bus_id_drop;
                  $final_bus_ids[]= $bus_id_drop;
                  $final_student_ids[]= $student_ids[$i];
                  $card_number[]=$studentcard_number;
                  $student_names[]=$student_name;
                }
...
// to get imei nos 
for($l=0;$l<sizeof($final_bus_ids);$l++)
{
  $query1="SELECT device_imei_no from vehicle_assignment where vehicle_id='$final_bus_ids[$l]' AND status='1'";
   ...
   $vserial[]= $row1->device_imei_no;
  }
}
//echo'bus_ids : '.$bus_id_string;
      ...
      $query2="SELECT * from bus_assignment where bus_serial=$bus_id AND school_id=$school_id AND status='1'";
        ...
        {									
          ...
          if($row_result4!=null)
          {
            ...
            //echo"ShiftName=".$shift_name;
            $shift_starttime = $result5->shift_starttime;
            //echo"shift_start".$shift_starttime;
            $shift_stoptime = $result5->shift_stoptime;
            //echo"shift_stop".$shift_stoptime;
          }
...
if($vsize>0)
{
 ...
 // $absolute_path_temp="C:\\xampp/htdocs/schoolvts";
 // $xmltowrite = $absolute_path_temp."/xml_tmp/filtered_xml/tmp_".$current_dt.".xml";
  //$xmltowrite = $absolute_path_temp."/xml_tmp/filtered_xml/tmp_11.xml";
  //echo "filepath". $xmltowrite;
  write_distance_report_xml($vserial,$card_number,$date1, $date2, $user_interval, $xmltowrite);
}

function write_distance_report_xml($vserial,$card_number, $startdate, $enddate, $user_interval, $xmltowrite)
{
  global $DbConnection;
  $maxPoints = 1000;
	$file_exist = 0;
  ...
  //echo"xmlfile=".$xmltowrite;
    ...
    //echo   "card_number".' '.$card_number[$i];
    get_distance_xml_data($vserial[$i],$card_number[$i], $vname[$i], $startdate, $enddate, $user_interval, $xmltowrite);
    //echo   "t2".' '.$i;
	//echo "Card S=".$card_number[$i];
	}  
...
function get_distance_xml_data($vehicle_serial, $card_serial, $vehicle_name, $startdate, $enddate, $user_interval, $xmltowrite)
{
    echo "Sizeof".sizeof(card_serial);
	global $busstop_ids;
  ...
  global $route_ids;
  global $bus_serials;
  global $final_bus_ids;
  global $final_student_ids;
  global $card_number;
             ...
             // echo "test7".'<BR>';
              ...
              if($status==0)
              {
               // continue;
      				...
      				$rfid_tmp1 = explode("=",$rfid_tmp[0]);
      				$rfid2 = preg_replace('/"/', '', $rfid_tmp1[1]);
              
              $CurrentLat = $lat2;
      	      $CurrentLong = $lng2;   
              //echo "sizeofc=".sizeof($card_serial);
              //echo"<br>rfid2 = ".$rfid2;    				
		          $card_number=$card_serial;
              // to calculate bus values
              echo "sizeofc=".sizeof($card_number);
              for($j=0;$j<sizeof($card_number);$j++)
              {
               //echo "CardNO;".$card_serial;
               //echo"<br>Count".$i;
			         //echo"<br>size of card :".sizeof($card_number);
			         //echo "vserial".$vserial;
			         //echo"BUS SER=".$bus_serials[$i];
                if($bus_serials[$i]==$vserial)
                {
                    ...
                    //if($distance < 0.025)
                    /*if($old_card_number != $rfid2 )
                    {
                    $old_card_number = $rfid2;
                    //echo"<br>old_card_number :".$i."  ".$old_card_number;
                    //echo"<br>rfid2 :".$i."  ".$rfid2;
                    
                    if($card_number[$j]==$rfid2)
                    {
                    */
                    //echo"<br>old_card_number :".$i."  ".$old_card_number;
                    //echo"<br>rfid2 :".$i."  ".$rfid2;
							...
							for($kk=0;$kk<sizeof($busstop_ids);$kk++)
								{
								calculate_distance($CurrentLat, $latitudes[$i], $CurrentLong, $longitudes[$i], &$distance);	
									if($kk==0)
									{
										$min_distance = $distance;
										$finalStop = $busstop_names[$kk];
									}
							...
							{							
								...
								if($xml_date > $time_start && $xml_date < $time_stop)
								{
									...
									$studentname=$student_names[$m];
									//echo"<br>shiftname :".$i."  ".$shiftname;
								}
							
								//	echo"<br>xmldate :".$xml_date;   
							}
							...
							if($shiftname!="")
							{
								...
								if($old_card_number != $rfid2 )  
								{
									...
									//echo"<br>old_card_number :".$i."  ".$old_card_number;
									//echo"<br>rfid2 :".$i."  ".$rfid2;
									if($card_number==$rfid2)
									//if($card_number[$j]==$rfid2)
									{
									$bus_data = "\n<marker vname=\"".$vname."\" sname=\"".$student_names[$i]."\" date=\"".$date1[0]."\" busstop=\"".$busstopname."\" arrival=\"".$date1[1]."\"  scheduledtime=\"".$arrivaltime."\"  shift=\"".$shiftname."\"  RFID=\"".$rfid2."\"/>";		
									$old_card_number = $rfid2;
                   ...
                   /* 
                    echo'<br>vname = '.$vname;
          					echo' date = '.$date1[0];
          					echo' busstop_name = '.$busstop_names[$i];
          					echo' arrival_time = '.$arrival_times[$i];
          					echo' shift_name = '.$shift_names[$i];
          					*/
						
						//}// if for rfid check closed						
						
          			//}//if for old card closed
...
echo '<center>';
  ...
  read_student_xml($xml_path, &$vname, &$sname, &$datefor, &$busstop, &$arrival, &$scheduledtime, &$shiftname);
  ...
  for($i=0;$i<sizeof($vname);$i++)   
    ...
    if(($i==0) || (($i>0) && ($vname[$i-1] != $vname[$i])) )
    {
      ...
      $sno = 1;
      $title="Student Report : ".$sname[$i]." -From DateTime : ($date1) and ($date2)";
      $vname1[$j][$k] = $vname[$i];
    ...
    echo'<td class="text" align="left">'.$sname[$i].'</td>';		
 ...
 echo'<br><br>';
	...
	echo'<form method = "post" action="src/php/report_getpdf_type4.php?size='.$ssize.'" target="_blank">';
	
	for($x=0;$x<=$j;$x++)
	{								
		$title=$sname[$x].": Performance Report From DateTime : ".$date1."-".$date2;
      ...
      $shiftnametmp = $shiftname1[$x][$y];	
			...
			echo"<input TYPE=\"hidden\" VALUE=\"$snametmp\" NAME=\"temp[$x][$y][Student Name]\">";
      ...
      echo"<input TYPE=\"hidden\" VALUE=\"$arrivaltmp\" NAME=\"temp[$x][$y][Arrival Time]\">";
			//echo"<input TYPE=\"hidden\" VALUE=\"$scheduledtimetmp\" NAME=\"temp[$x][$y][Scheduled Time]\">";
