...
if($numrows)
{
  ...
  $query = "SELECT DISTINCT device_imei_no FROM vehicle_assignment WHERE vehicle_id IN (SELECT vehicle_id FROM vehicle_grouping WHERE ".
            "account_id='$account_id' AND status=1) AND status=1";
  ...
  if (file_exists($xml_file)) 
    ...
    copy($xml_file,$xml_original_tmp);
      ...
      while(!feof($fp)) 
            ...
            $current_time = date("Y-m-d H:i:s");	
            $halt_time = strtotime($current_time) - strtotime($last_halt_time);
            //echo  "<br>halttime=".$halt_time.", current_time=". $current_time." ,lasthatltime=".$last_halt_time;
            $hms_2 = secondsToTime($halt_time);
            $halt_time_formated = $hms_2[h]."/".$hms_2[m]."/".$hms_2[s];
            //echo "Formated=".$halt_time_formated;
            //echo ' IMEI='.$imei.':Vehicle='.$vname.':CurrentTime='.$datetime.':Lat='.$lat.':Lng='.$lng.':HaltTime='.$halt_time_formated.':Speed='.$speed.':Location='.$place;                                                                    
            echo ' #IMEI~'.$imei.'^Vehicle~'.$vname.'^CurrentTime~'.$datetime.'^Lat~'.$lat.'^Lng~'.$lng.'^HaltTime~'.$halt_time_formated.'^Speed~'.$speed.'^Location~'.$place;
				}																			
		 }
     echo "^DATAEND";
