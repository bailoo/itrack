...
//include_once("get_filtered_xml.php");      // WRITE SORTED XML , FINAL XML NAME STORED IN 'xmltowirte' VARIABLE
//include_once("read_filtered_xml.php");
//echo "xmltowrite=".$xmltowrite." startdate=".$startdate." enddate=".$enddate;
include_once("getLastPositionXml.php");      // WRITE SORTED XML , FINAL XML NAME STORED IN 'xmltowirte' VARIABLE
?>
...
<script language="javascript" src="../js/jquery.js"></script>
 ...
 var map;
	...
	function GeocodeNext()
	{
		...
		function addLocation(latLng, result, distance,user_lat,user_lng,deviceimeino) 
		{
			...
			addResult(latLng, address, result.formatted_address, distance,user_lat,user_lng,deviceimeino);
		}
			
		function addResult(latLng, address, formattedAddress, distance,user_lat,user_lng,deviceimeino) {
     // alert("latLng"+latLng);
      //document.getElementById("geocodedPostcodes").value +=
		...
		if(xml_address_flag==0)
		{
			...
			{
				address2=distance + " km from " + address +"#"+deviceimeino+ ":"; 				
...
echo'
  <form method="post" action="LastData.htm" target="_self">
    
  <div id="progress"></div>
  <div id="delay"></div>';
	/*if($account_id=="212")
	{
		echo'<textarea id="geocodedPostcodes" name="geocodedPostcodes" cols="40" rows="20" ></textarea>';
	}
	else
	{
...
{	
 ...
 $point = '"';
  for($i=0;$i<$size;$i++)
    ...
    if($i==0)
    {
      $point = $point.$coord.','.$vehicleserial[$i];    
    }
    else
    {
      $point = $point.":".$coord.','.$vehicleserial[$i];   
...
echo'
	...
	<input type="hidden" name="vehiclenumber" value='.base64_encode(serialize($vehiclenumber)).'>
	<input type="hidden" name="vehicletype" value='.base64_encode(serialize($vehicletype)).'>
