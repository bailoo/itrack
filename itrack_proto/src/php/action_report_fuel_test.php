<?php
	include_once('lib_test/BUG.php');
	include_once('lib_test/Util.php');
	include_once('lib_test/VTSFuel.php');
	include_once("report_title.php");
  include_once('lib_test/VTSMySQL.php'); 
  ...
  $startDateTime = str_replace("/","-",$_POST['start_date']);
	...
	$tmp = "975";  //359231037645452  (BH_7508_504)
	
	//$tmp = "311";
  $imeis = explode(":",$tmp); 
	$startDateTime = "2012-03-06 00:00:00";
	//$endDateTime = "2012-03-06 00:30:01";
	$endDateTime = "2012-03-06 13:03:55";
	$timeInterval = "30";
  ...
  foreach($imeis as $imei)
	{ 	      
	  $fuel_data = VTSFuel::getFuelData($DbConnection, $imei, $startDate, $endDate1);		
		echo "<br>fuel_data=".$fuel_data[$imei];
			...
			foreach($time_list as $datetime)
			{
				if(strtotime($datetime) <= strtotime($datetime_now))
				{
					...
					if($fuel_datetime >= 0)
					{
						...
						//echo "<br>fuel[imei][datetime]=".$fuel_datetime;
					}
				}
			}
		}		
        ...
        foreach($imeis as $imei)
        {
          ...
          if($fuel_size)
          {
            ...
            echo '<center>           
                    ...
                    <tr>							
                      <td class="text" align="left" width="50px;"><b>SNo</b></td>
                    ...
                    {                                                                                         
                      ...
                      if($fuel_imei<1000)
                      {
                        ...
                        echo'<tr>
                  			...
                  			echo"<input TYPE=\"hidden\" VALUE=\"$datetime\" NAME=\"temp[$x][$y][DateTime]\">";
                  			echo"<input TYPE=\"hidden\" VALUE=\"$fuel_imei_1\" NAME=\"temp[$x][$y][Fuel level(litres)]\">";      			
  ...
  echo'</div>
        <table align="center">
          <tr>
            ...
            if($imei_size==0)
            {						
              print"<center><FONT color=\"Red\" size=2><strong>No Fuel Record found</strong></font></center>";              
              echo'<br><br>';
            }	
            else
            {
