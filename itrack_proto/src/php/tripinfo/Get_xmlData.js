...
function loadCurrentXML(xmlFile)
{
	var xmlhttp=false;
	/*@cc_on @*/
...
function modify_xml_text(textStr)
{
	//alert(textStr.length);
	//for(var i=0;i<textStr.length;i++)
	//{
		var str2 = textStr.split("\n");
		var strlen;
		var k=0;		
		
		for(var j=0;j<str2.length-1;j++)
		{	
			strlen = str2[j].length;
			//alert("str2["+j+"] len ="+strlen);
			
			//alert("strj0="+str2[j][0]);
			//if((str2[j].charAt(0)=='<') && (str2[j].charAt(strlen-1)=='>'))
			
			if((str2[j].charAt(0)=='<') && (str2[j].charAt(strlen-1)=='>')&& (str2[j].charAt(1)!='/'))			
			{
			 str2[k] = str2[j];
			 k++;
			}
			...
			str2.length = k+1;
			//str2[str2.length-1] = "";
			var finalStr = str2.join("\n");
			//alert(finalStr);
			/*str2[str2.length-2] = str2[str2.length-2]+"</t1>";
			str2.length = str2.length -1;
			//str2[str2.length-1] = "";
			var finalStr = str2.join("<m");
			//alert(finalStr);*/						
	//}
