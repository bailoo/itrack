		...
		foreach($dateList as $i => $date)
		{
			foreach($xmlDirs as $xmlDir)
			{
				...
				if(file_exists($file))
				{
					...
					if($xml)
					{
						set_master_variable($date);
						while(!feof($xml))
						{
							$line = fgets($xml, 4096);							
							if(strpos($line,"x"))
							{
								$datetime = UTIL::getXMLData('/'.$vh.'="[^"]+"/', $line);
								...
								if(($datetimeTS>=$datetimeStartTS) && ($datetimeTS<=$datetimeEndTS))
								{
									...
									for($fieldIndex = 0 ; $fieldIndex < $fieldsCount ; $fieldIndex++)
									{
										...
										if($field=="latlng")
										{
											$lat = UTIL::getXMLData('/'.$vd.'="\d+\.\d+[NS]\"/', $line);
											$lng = UTIL::getXMLData('/'.$ve.'="\d+\.\d+[EW]\"/', $line);
                    ...
                    for($fieldIndex = 0 ; $fieldIndex < $fieldsCount ; $fieldIndex++)
                      ...
                      if($io_status[1]!="")
                      {
						...
						$fuel_voltage_io=get_io_to_new_method($date,$old_xml_date,$fuel_voltage_io);
