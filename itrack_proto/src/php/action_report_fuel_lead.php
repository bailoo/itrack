<?php
	...
	if($vsize>0)
	{
		...
		write_fuel_lead_report_xml($vserial, $vname, $date1, $date2, $xmltowrite);
	}

function write_fuel_lead_report_xml($vserial, $vname, $startdate, $enddate, $xmltowrite)
{
	$maxPoints = 1000;
	$file_exist = 0;
	...
	{  	
		...
		get_fuel_lead_xml_data($vserial[$i], $vname[$i], $startdate, $enddate, $xmltowrite);
		//echo   "t2".' '.$i;
	}  
...
function get_fuel_lead_xml_data($vehicle_serial, $vname, $startdate, $enddate, $xmltowrite)
{
	global $va,$vb,$vc,$vd,$ve,$vf,$vg,$vh,$vi,$vj,$vk,$vl,$vm,$vn,$vo,$vp,$vq,$vr,$vs,$vt,$vu,$vv,$vw,$vx,$vy,$vz,$vaa,$vab;
	global $old_xml_date;
	global $DbConnection;
	$io = get_io($vehicle_serial,'fuel_lead');
	//echo "io=".$io."<br>";
  				...
  				{				  
					...
					//$lat = $lat_value[1] ;
  					...
  					{							           	
						...
						$status = preg_match('/'.$io.'=\"[^\"]+/', $line, $fuel_lead_tmp);
						/*if($_SERVER['HTTP_X_FORWARDED_FOR'] == "172.26.48.181")
						...
						$fuel_lead_tmp1 = explode("=",$fuel_lead_tmp[0]);  
						$fule_lead_count = preg_replace('/"/', '', $fuel_lead_tmp1[1]);                                                                            	                         
						//echo "<br>enginecount=".$engine_count;

						//$date_secs2 = strtotime($time2);                         	
						//if($_SERVER['HTTP_X_FORWARDED_FOR'] == "172.26.48.15")
						//echo "<br>engine_count=". $engine_count." ,c=".$c." ,total_lines=".$total_lines;              
						if($fule_lead_count>350)
						{
							$continuous_running_flag = 1;
						}

						if($fule_lead_count>350 && $StartFlag==0)  //500
						{                						
							$time1 = $datetime;
							$StartFlag = 1;
						} 
						else if( ($fule_lead_count<350 && $StartFlag==1) || ( ($c==($total_lines-1)) && ($continuous_running_flag ==1) ) )   //500
						{
							$StartFlag = 2;
						}
						$time2 = $datetime;
						...
						if($StartFlag == 2)
						{
						  ...
						  //echo "<br>runtime=".$runtime;
							...
							$fuel_lead_data = "\n<marker imei=\"".$vserial."\" vname=\"".$vname."\" datefrom=\"".$time1."\" dateto=\"".$time2."\" fuel_lead=\"".$runtime."\"/>";						          						
							//echo "<br>".$engine_runhr_data;
							$linetowrite = $fuel_lead_data; // for distance       // ADD DISTANCE
...
$m1=date('M',mktime(0,0,0,$month,1));
  ...
  echo'<center>';   
  report_title("Fuel Lead Report",$date1,$date2);   
	echo'<div style="overflow: auto;height: 285px; width: 600px;" align="center">';
   
  ///////////////////  READ SPEED XML 	//////////////////////////////				                      
  $xml_path = $xmltowrite;
  //echo "<br>xml_path=".$xml_path;
	read_fuel_lead_xml($xml_path, &$imei, &$vname, &$datefrom, &$dateto, &$fuel_lead);
  ...
  for($i=0;$i<sizeof($imei);$i++)
    ...
    if(($i==0) || (($i>0) && ($imei[$i-1] != $imei[$i])) )
    {
      ...
      <tr>
			...
			<td class="text" align="left"><b>Fuel Lead (hr:min:sec)</b></td>								
      </tr>';  								
    }                                                                        				
    
    $sum_fuel_lead = $sum_fuel_lead + $fuel_lead[$i];
    ...
    $hms1 = secondsToTime($fuel_lead[$i]);
    ...
    {       
        ...
        //if($k>1)
  			{        
          $hms_2 = secondsToTime($sum_fuel_lead);                   
          $total_fuel_lead[$j] = $hms_2[h].":".$hms_2[m].":".$hms_2[s];
        ...
        //echo'<td class="text"><font color="red"><strong>'.$total_engine_runhr[$j].'</strong></font></td>';        
  			...
  			echo'<td class="text"><font color="red"><strong>'.$total_fuel_lead[$j].'</strong></font></td>';
  			echo'</tr>'; 
        echo '</table>';
...
if($single_data_flag)
{
  ...
  for($x=0;$x<=$j;$x++)
	{								
		$title = $vname1[$x][0]." (".$imei1[$x][0]."): Fuel Lead Report- From DateTime : ".$date1."-".$date2;
		$csv_string = $csv_string.$title."\n";
    $csv_string = $csv_string."SNo,Start Time From,End Time,Fuel Lead(hrs.min)\n";
    ...
    for($y=0;$y<=$no_of_data[$x];$y++)
		{
			...
			echo"<input TYPE=\"hidden\" VALUE=\"$fuel_lead_tmp\" NAME=\"temp[$x][$y][Fuel Lead (hr:min:sec)]\">";
      
      $csv_string = $csv_string.$sno.','.$datetmp1.','.$datetmp2.','.$fuel_lead_tmp."\n";         																	
		}		
    ...
    echo"<input TYPE=\"hidden\" VALUE=\"\" NAME=\"temp[$x][$y][SNo]\">";
		...
		echo"<input TYPE=\"hidden\" VALUE=\"\" NAME=\"temp[$x][$y][Fuel Lead (hr:min:sec)]\">";									
		...
		echo"<input TYPE=\"hidden\" VALUE=\"$total_fuel_lead[$x]\" NAME=\"temp[$x][$m][Fuel Lead (hr:min:sec)]\">";
		$csv_string = $csv_string."\nTotal,".$date1.",".$date2.",".$total_fuel_lead[$x]."\n\n";       
  ...
  echo'	
    <table align="center">
		<tr>
  		...
  		if(sizeof($imei)==0)
  		{						
  			print"<center><FONT color=\"Red\" size=2><strong>Sorry! No Fuel Lead Record Found</strong></font></center>";
  			echo'<br><br>';
  		}	
  		else
  		{
        echo'<input TYPE="hidden" VALUE="fuel_lead" NAME="csv_type">';
