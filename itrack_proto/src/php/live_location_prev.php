      ...
      //document.getElementById("geocodedPostcodes").value +=
		...
		if(xml_address_flag==0)
		{
			...
			if((address.indexOf("NH") ==-1) && (address.indexOf("National Highway") ==-1) && (address.indexOf("State Highway") ==-1) && (address.indexOf("SH") ==-1))
			{
				...
				//alert("vehTdStr="+vehTdStr);
				 if (document.getElementById(vehTdStr.trim()).innerHTML.indexOf("LM") ==-1)
				 {
				// alert("in if");
					geocodedPostcodes += address2;
					document.getElementById(vehTdStr.trim()).innerHTML=address2;
				 }
				 else
				 {
					// alert("in else");
...
for($i=0;$i<sizeof($vehiclename_arr_last);$i++)
{
		...
		{
		echo '<td><span class="style_v7">'.$vehiclename_arr_last[$i].'</span></td>';
		}
		else if($vehilce_status_arr[$i]=="Stopped")
		{
		echo '<td><span class="style_v6">'.$vehiclename_arr_last[$i].'</span></td>';
		}
		...
		{
			echo '<td><span class="style_v9" id=\''.$vserial_arr_last[$i].'\'>'.$landmark_last[$vehiclename_arr_last[$i]].'-(LM)</span></td>';
		}
		else
		{
			echo '<td><span class="style_v9" id=\''.trim($vserial_arr_last[$i]).'\'>'.$lacStr[$i].'</span></td>';
		}
		...
		{
			echo'<td><span class="style_v12"><strong>Stopped</strong></span></td>';
		}		
	...
	{
		$point = $point.$coord.":";
		$vehStr=$vehStr.$vserial_arr_last[$i].":";
	}
	else
	{
