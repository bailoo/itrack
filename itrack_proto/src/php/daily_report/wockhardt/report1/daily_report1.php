...
/////********* CREATE EXCEL FILE *******************///////
$rno = rand();
$filename_title = 'DAILY_WOCKHARDT_VTS_REPORT_1_'.$rno;
//ONLINE
      ...
      //{
        ...
        for($y=0;$y<=($date_size-1);$y++)                       //CHECKS EVERYTIME FOR ONE RECORD AND BREAKS- OUTER LOOP
          ...
          for($z=0;$z<sizeof($day_db);$z++)                     //CHECKS EVERYTIME FOR ONE RECORD AND BREAKS- INNER LOOP
          {
            ...
            {                            
              //echo "\nDAY SPECIFIED";
              add_data_to_field($imei[$x], $vname[$x], $date1, $date2, $op_date1, $op_date2, $geo_point, $base_station_coord[$x], $border1, $border2, $worksheet);          
              $wflag = 1;
              break;
            }
          }       	 
...
function add_data_to_field($imei, $vname, $date1, $date2, $op_date1, $op_date2, $geo_point, $base_station_coord, $border1, $border2, $worksheet)
{      
    global $worksheet;
    global $r;
    global $c;
