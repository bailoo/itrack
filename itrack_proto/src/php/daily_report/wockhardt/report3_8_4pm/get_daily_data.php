...
function get_daily_data($imei, $vname, $startdate, $enddate)
{
	...
	//echo "\nFUNCTION1::imei=".$imei.",vname=".$vname.",startdate=".$startdate.",enddate=".$enddate;  
	//echo "\ndebug1:";
		...
		{			
			...
			{      
				...
				while(!feof($xml))                // WHILE LINE != NULL
				{
					...
					//echo "\nFinal0=".$xml_date." datavalid=".$DataValid;
	...
	$nodata_string = "-";
	$nomovement_string ="-";  
	//echo "\nNoDeviceReportingFlag=".$nodevice_reporting_flag;
	
	if($nodevice_reporting_flag)                //NO DEVICE REPORTING
	{
	  //$nodata_string = $datefrom; 
	  $nodata_string = $startdate." to ".$enddate;
	}
	else
	{
		if( ($total_dist < 1) )    // >1 KM       //NO DEVICE MOVEMENT
		{
		  ...
		  $nomovement_string = $startdate." to ".$enddate;
		}
	}	
