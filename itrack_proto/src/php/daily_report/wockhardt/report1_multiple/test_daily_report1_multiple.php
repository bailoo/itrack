...
include_once("test_get_daily_data_multiple.php");
...
$date = date('Y-m-d');
//$previous_date = date('Y-m-d', strtotime($date .' -1 day'));	//COMMENT IT
$previous_date = date('Y-m-d', strtotime($date .' -2 day'));    //COMMENT IT
//$previous_date = $date; //UNCOMMENT
...
$query = "SELECT DISTINCT vehicle_assignment.device_imei_no, schedule_assignment.vehicle_id, schedule_assignment.min_operation_time, ".
        ...
        "schedule_assignment.status=1 AND vehicle_assignment.status=1 AND schedule_assignment.location_id AND NOT(schedule_assignment.location_id='')";
...
echo "\nVSizeTotal=".$y." ,vsize=".sizeof($imei)." ,SizeBSCoord=".sizeof($base_station_coord);
      ...
      //{
        ...
        for($y=0;$y<=($date_size-1);$y++)                       //CHECKS EVERYTIME FOR ONE RECORD AND BREAKS- OUTER LOOP
          ...
          for($z=0;$z<sizeof($day_db);$z++)                     //CHECKS EVERYTIME FOR ONE RECORD AND BREAKS- INNER LOOP
          {
            ...
            {                            
              //echo "\nDAY SPECIFIED";
              add_data_to_field($imei[$x], $serial, $vname[$x], $date1, $date2, $op_date1, $op_date2, $geo_point1, $base_station_coord[$x], $base_station_name[$x], $border1, $border2, $worksheet);          
              $wflag = 1;
              break;
            }
          }       	 
...
function add_data_to_field($imei, $serial, $vname, $date1, $date2, $op_date1, $op_date2, $geo_point1, $base_station_coord, $base_station_name, $border1, $border2, $worksheet)
{      
    global $worksheet;
    global $r;
    global $c;
    ...
    echo "\nBeforegetData";
     $daily_data = test_get_daily_data($imei, $vname, $date1, $date2, $op_date1, $op_date2, $geo_point, $base_station_coord);
    echo "\nAfterGetData";
    //#######################################//    
