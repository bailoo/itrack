...
function get_daily_data($imei, $vname, $startdate, $enddate, $op_date1, $op_date2, $min_halt_time, $max_halt_time, $nonpoi_halt_time, $geo_point, $geo_name)
{
	...
	{   
		...
		$xml_current = $back_dir."/xml_vts/xml_data/".$userdates[$i]."/".$imei.".xml";	    		
		...
		{
			$xml_file = $back_dir."/sorted_xml_data/".$userdates[$i]."/".$imei.".xml";
			$CurrentFile = 0;
		}
							...
							*/
							...
							for($k=1;$k<=1;$k++)
							{
								...
								{           
									...
									{            			
										//echo "\nSet flag halt:halt_flag=".$halt_flag[$k]." ,k=".$k;
										$halt_flag[$k] = 1;
									}					                              
								}  //ELSE CLOSED
	...
	for($k=0;$k<sizeof($geo_point);$k++)
	{
		...
		if($valid_location[$k] == 1)
		{
			...
			echo "\ntotal_halt_time[k]=".$total_halt_time[$k]." ,min_halt_time_a[k]=".$min_halt_time_a[$k]." ,max_halt_time_a=".$max_halt_time_a[$k]." ,max_halt_sec=".$max_halt_sec;
			...
			if( ($total_halt_time[$k] < $min_halt_sec) || ($total_halt_time[$k] > $max_halt_sec) )
			{
				...
				{
					//echo "\nOne";
					$diff = ($total_halt_time[$k] - $min_halt_sec);
				}
				else
				{
					//echo "\nTwo";
