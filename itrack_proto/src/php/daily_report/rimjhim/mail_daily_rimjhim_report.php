...
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
        ...
        "vehicle_grouping.account_id=449 AND vehicle.status=1 AND vehicle_grouping.status=1 AND vehicle_assignment.status=1";                                                          
...
if($vehicle_id_final!="")
{
	...
	//$inc_serial=$i+1;
	$inc_serial = rand();
	$filename_title = 'VTS_DAILYREPORT_RIMJHIM_'.$previous_date."_".$inc_serial;
	...
	{		
		...
		for($q=0;$q<sizeof($sheet1_row);$q++)
		{
			...
			//echo "\nsheet1_data=".sizeof($sheet1_data);
	...
	//echo "\nSheet1DataMainStr=".$sheet1_data_main_string;
	//echo "\nCSV_stringhaltfinal=".$csv_string_halt_final;

	$worksheet2 =& $workbook->addworksheet('Halt Report');
	$sheet2 = explode('@',$csv_string_halt_final);
	$r=0;   //row 
			
	//echo "\nSheet2Size=".sizeof($sheet2);
	...
	for($p=0;$p<sizeof($sheet2);$p++)
	{
		...
		for($q=0;$q<sizeof($sheet2_row);$q++)
		{
			...
			//echo "\nsheet2_data=".sizeof($sheet2_data);
	...
	*/
	...
	$to = 'rnsingh@rimjhimispat.com,caricha29@gmail.com'; 
	$subject = 'VTS_REPORT_RIMJHIM_'.$previous_date;
	$message = 'VTS_REPORT_RIMJHIM_'.$previous_date; 
