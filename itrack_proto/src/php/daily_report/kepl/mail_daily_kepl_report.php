...
/////********* CREATE EXCEL FILE *******************///////
$rno = rand();
$filename_title = 'DAILY_KEPL_VTS_REPORT_'.$previous_date.'_'.$rno;
...
$query_account = "SELECT account_id,user_id FROM account WHERE account_id IN(select account_id from account_detail where account_admin_id = 313) and status=1"; //KEPL ACCOUNT_ADMIN_ID=313
...
while($row_account = mysql_fetch_object($result_account))
{
  ...
  echo "\nUSER::".$user_name." ,TOTAL VEHICLES::".$vsize."\n";  
...
function get_daily_rspl_report_xml($vehicle_serial, $vname, $startdate, $enddate)
{
  ...
  $subject = 'VTS_REPORT_KEPL_'.$previous_date."/".$previous_year;
  $message = 'VTS_REPORT_KEPL_'.$previous_date."/".$previous_year; 
