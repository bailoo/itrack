...
//include_once($abspath."/daily_report/motherdairy/halt2/bvm_get_master_detail.php");
...
// DELETE TEMP DATE
$previous_date = '2013-11-01';

$current_date = $previous_date;
///////////////////////////////////////////
//$startdate = $current_date." 00:00:00";      // MORNING TIME: 3AM TO 6PM
//$enddate = $current_date." 23:59:59"; 

$startdate = $previous_date." 00:00:00";      // MORNING TIME: 3AM TO 6PM
...
if($vsize>0)
{
  ...
  include_once("bvm_action_halt2.php"); 
  //echo "\nAfter"; 
...
if($vehicle_id_final!="")
{
	...
	//$inc_serial=$i+1;
	$inc_serial = rand();
	$filename_title = 'BVM_DAILY_REPORT_'.$previous_date."_".$inc_serial;	
	$fullPath = "/var/www/html/vts/beta/src/php/daily_report/bvm/report_log/".$previous_date;
	...
	{
			echo "\nFolder Doesnot exist:created";
			mkdir($fullPath, 0700);
	}
	
	$fullPath = $fullPath."/".$filename_title;
	...
	$worksheet2->write($r, 0, "BillingDate", $text_format);
	$worksheet2->write($r, 1, "Route", $text_format);
	$worksheet2->write($r, 2, "Sold To Party", $text_format);
	$worksheet2->write($r, 3, "VehicleNo", $text_format);
	$worksheet2->write($r, 4, "Google Location", $text_format);
	$worksheet2->write($r, 5, "IN TIME", $text_format);
	$worksheet2->write($r, 6, "OUT TIME", $text_format);
	$worksheet2->write($r, 7, "DURATION (H:m:s)", $text_format);
	
	
	$csv_string = "BillingDate,Route,Sold To Party,VehicleNo,Google Location,IN TIME,OUT TIME,DURATION (H:m:s)\n";
	
	$r++;		
	//echo "\nSheet=".sizeof($sheet2);
    $route="-";
	$newDate = date("d-m-Y", strtotime($previous_date));	
	...
	{				
		...
		{
			$c=0;
			$worksheet2->write($r,$c, $newDate);
			$csv_string = $csv_string.$newDate;
			$c++;
				
			$worksheet2->write($r,$c, $route);
			$csv_string = $csv_string.",".$route;
			$c++;	
			...
			{           
				...
				{					 
					...
					$csv_string = $csv_string.",".$sheet2_data[$m];
					$c++;
    ...
    {    
      ...
      if(!$found)
      {
        ...
        $worksheet2->write($r, 1, "", $text_format);
		//$worksheet2->write($r, 2, "", $text_format);
		$worksheet2->write($r, 2, $point[$m], $text_red_format);
  ...
  $to = 'Shreya.Shinde@motherdairy.com,Vijay.Singh@motherdairy.com,Ravindra.Negi@motherdairy.com,ashish@iembsys.co.in';
