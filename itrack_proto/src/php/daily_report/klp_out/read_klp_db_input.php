...
while($row_vehicle = mysql_fetch_object($result_vehicle))
{
	...
	$query_icd = "SELECT sno,vehicle_name,container_no,icd_out_datetime,icd_code,icd_in_datetime,factory_ea_datetime,".
	...
	while($row=mysql_fetch_object($result))
	{
		...
		//if($row->vehicle_name=='UP78CN8183')		
		//if($row->vehicle_name=='UP78CN7920' || $row->vehicle_name=='UP78CN7931' || $row->vehicle_name=='HR55R8192' || $row->vehicle_name=='UP78CN7564')
		{
			//echo "\nFOUND DATA";
			...
			//{
				...
				{							
					if(trim($row->vehicle_name) == trim($vehicle_name_db[$i]))
					{
						...
						$customer_name_tmp = $factory_string_tmp[0];
