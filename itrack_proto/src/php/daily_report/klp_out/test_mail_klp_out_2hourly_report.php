...
include_once("test_read_klp_db_input.php");
...
{	
	...
	//###### GET FACTORY DETAIL FROM DATABASE
	$query_factory = "SELECT factory_in_datetime,factory_out_datetime,current_location,distance_travelled,report_status,".
	"delay,halt FROM icd_input WHERE sno='$sno[$i]'";
	$result_factory = mysql_query($query_factory,$DbConnection);
	if($row_factory = mysql_fetch_object($result_factory))
	{
		$factory_in_datetime_db = $row_factory->factory_in_datetime;
		$factory_out_datetime_db = $row_factory->factory_out_datetime;
		
		if($factory_in_datetime_db=='0000-00-00 00:00:00')
		{
			$factory_in_datetime_db = '';
		}
		
		if($factory_out_datetime_db=='0000-00-00 00:00:00')
		{
			$factory_out_datetime_db = '';
		}		
		
		$factory_in_datetime_db1 = explode(' ',$factory_in_datetime_db);
		$factory_out_datetime_db1 = explode(' ',$factory_out_datetime_db);
		
		$factory_in_date_db = $factory_in_datetime_db1[0];
		$factory_in_time_db = $factory_in_datetime_db1[1];
		$factory_out_date_db = $factory_out_datetime_db1[0];
		$factory_out_time_db = $factory_out_datetime_db1[1];
 
		$placename1 = $row_factory->current_location;
		$total_dist_res	= $row_factory->distance_travelled;	
		$status_res	= $row_factory->report_status;	
		$time_delay_res	= $row_factory->delay;	
		$factory_halt_time_res = $row_factory->halt;		
	}
	//echo "\nICDIN=".$icd_indate[$i];
	if($icd_indate[$i]!="")
	{
...
echo "\nICD_IN_DATE=".$icd_indate[$i];
	
	if($icd_indate[$i]!="")
	{
		echo "\nIN";
		if($factory_in_time_db!="" && $factory_out_date_db!="" && $placename1!="" && $total_dist_res!="" && $status_res!="" && $time_delay_res!="" && $factory_halt_time_res!="")
		{
			echo "\nIN-1";
			$factory_in_date = $factory_in_date_db;
			$factory_in_time = $factory_in_time_db;
			$factory_out_date = $factory_out_date_db;
			$factory_out_time = $factory_out_time_db;
		}
		else
		{
			echo "\nIN-2";
			...
			$factory_in_date = $factory_in_time_res1[0];
			$factory_in_time = $factory_in_time_res1[1];
			$factory_out_date = $factory_out_time_res1[0];
			$factory_out_time = $factory_out_time_res1[1];
				
			$schedule_in_time_res = $daily_data_tmp[6];
			$time_delay_res = $daily_data_tmp[7];
			$factory_halt_time_res = $daily_data_tmp[8];
			
			//####### GET GOOGLE LOCATION-2
			//echo "\nCOORD1=".$current_coord_res;
			...
			//####### UPDATE FACTORY ENTRIES IN DATABASE
			$factory_in_tmp = $factory_in_date." ".$factory_in_time;
			$factory_out_tmp = $factory_out_date." ".$factory_out_time;
			$query_factory_update = "UPDATE icd_input SET factory_in_datetime='$factory_in_tmp',factory_out_datetime='$factory_out_tmp',".
			"current_location='$placename1',distance_travelled='$total_dist_res',report_status='$status_res',delay='$time_delay_res',".
			"halt='$factory_halt_time_res' WHERE sno='$sno[$i]'";
			$result_factory_update = mysql_query($query_factory_update,$DbConnection);			
		}
	}
	else
	{
		echo "\nElse:1";		
		if($factory_out_date_db!="")
		{
			echo "\nElse:2";
			$factory_in_date = $factory_in_date_db;
			$factory_in_time = $factory_in_time_db;
			...
			$startdate = $factory_out_date." ".$factory_out_time;
			$enddate = $current_time;
			...
			//$icd_out_time_res1 = explode(' ',$daily_data_tmp[0]);
			$current_coord_res = $daily_data_tmp[1];
			$total_dist_res = $total_dist_res + $daily_data_tmp[2];
			if($status_res!="At ICD")
			{
				$status_res = "On the way to ICD";
			}			
			//####### GET GOOGLE LOCATION-1
			//echo "\nCOORD2=".$current_coord_res;
			$coord_tmp = explode(",",$current_coord_res);
			$lttmp = $coord_tmp[0];
			$lngtmp = $coord_tmp[1];
		...
		else
		{
			...
			if($lttmp!="" && $lngtmp!="")
			{
				...
				if($landmark!="")
				{
					//echo "<br>LNMRK1=".$landmark;
					$placename1 = $landmark;
				}
				else
				{			
					get_report_location($lttmp,$lngtmp,&$placename1);
					//echo "<br>PLB1=".$placename1;
					$placename1 = preg_replace('/भारत गणराज्य/', '' , $placename1);
					$placename1 = preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '' , $placename1);
					//echo "<br>PLB2=".$placename1;
				}
			}
	...
	$objPHPExcel->getActiveSheet()->getStyle($cell)->applyFromArray($styleBgData);	
	$cell = 'G'.$r;
	$objPHPExcel->getActiveSheet()->getCell($cell)->setValue($factory_in_date);
	$objPHPExcel->getActiveSheet()->getStyle($cell)->applyFromArray($styleBgData);

	//echo "\nC7";
	$cell = 'H'.$r;
	$objPHPExcel->getActiveSheet()->getCell($cell)->setValue($factory_in_time);
	$objPHPExcel->getActiveSheet()->getStyle($cell)->applyFromArray($styleBgData);

	//echo "\nC8";
	$cell = 'I'.$r;
	$objPHPExcel->getActiveSheet()->getCell($cell)->setValue($factory_out_date);
	$objPHPExcel->getActiveSheet()->getStyle($cell)->applyFromArray($styleBgData);	
	$cell = 'J'.$r;
	$objPHPExcel->getActiveSheet()->getCell($cell)->setValue($factory_out_time);
	$objPHPExcel->getActiveSheet()->getStyle($cell)->applyFromArray($styleBgData);
