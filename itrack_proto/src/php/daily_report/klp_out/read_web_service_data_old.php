...
else 
{
	...
	else 
	{
		...
		{
			//echo "vehicle_name=".$tmp_arr[$i]['VEHICLE_NO']." &nbsp;CUSTOMER=".$tmp_arr[$i]['CUSTOMER']." &nbsp;TARGET_IN_DATE=".$tmp_arr[$i]['TARGET_IN_DATE']." &nbsp;TARGET_OUT_DATE=".$tmp_arr[$i]['TARGET_OUT_DATE']." &nbsp;GATE_OUT_DATE=".$tmp_arr[$i]['GATE_OUT_DATE']." &nbsp;GATE_IN_DATE=".$tmp_arr[$i]['GATE_IN_DATE']."<br>";			
...
$query = "SELECT * FROM icd_input WHERE status=1";
...
if($num_rows==0)
{
	...
	for($i=0;$i<sizeof($input_vehicle_name);$i++)
	{
		...
		$query_insert = "INSERT INTO icd_input(vehicle_name,icd_code,icd_out_datetime,factory_code,factory_ea_datetime,factory_ed_datetime,icd_in_datetime,account_id,create_date,status,remark) values('$input_vehicle_name[$i]','$icd_code_global','$input_icd_out_datetime[$i]','$input_factory_code[$i]','$input_factory_ea_datetime[$i]','$input_factory_ed_datetime[$i]','$input_icd_in_datetime[$i]','$account_id','$current_date','$status','-')";
...
else
{
	...
	{
		$db_sno[] = $row->sno;
		$db_vehicle_name[] = $row->vehicle_name;
		$db_icd_code[] = $row->icd_code;
		$db_icd_out_datetime[] = $row->icd_out_datetime;
		$db_factory_code[] = $row->factory_code;
		$db_factory_ea_datetime[] = $row->factory_ea_datetime;
		$db_icd_in_datetime[] = $row->icd_in_datetime;
		$db_create_date[] = $row->create_date;
		$db_edit_date[] = $row->edit_date;
		$db_status[] = $row->status;
		$db_remark[] = $row->remark;	
	}
	...
	for($i=0;$i<sizeof($input_vehicle_name);$i++)
	{
		...
		for($j=0;$j<sizeof($db_vehicle_name);$j++)
		{
			...
			if( (trim($db_vehicle_name[$j]) == trim($input_vehicle_name[$i])) && ($db_icd_out_datetime[$j]!="0000-00-00 00:00:00") && ($db_icd_in_datetime[$j]=="0000-00-00 00:00:00") && ($input_icd_in_datetime[$i]!="0000-00-00 00:00:00") && ($db_icd_out_datetime[$j]==$input_icd_out_datetime[$i]))
			{
				...
				$db_icd_in_datetime[$i] = $update_icd_in_datetime;
				$flag_update = true;
				break;
			}
			//##### NEW ENTRIES :: INSERT
			...
			else if( (trim($db_vehicle_name[$j]) == trim($input_vehicle_name[$i])) && ($db_icd_out_datetime[$j]!="0000-00-00 00:00:00") && ($db_icd_out_datetime[$j] < $input_icd_out_datetime[$i]) && ($input_icd_out_datetime[$i]!="0000-00-00 00:00:00"))			
			{
				...
				{
					echo "\nMATCH::CLOSING : ICD_IN_TIME AND INSERTING NEW RECORD..".$i;
					//$insert_flag = true;				
					...
					$query_insert = "INSERT INTO icd_input(vehicle_name,icd_code,icd_out_datetime,factory_code,factory_ea_datetime,factory_ed_datetime,icd_in_datetime,account_id,create_date,status,remark) values('$new_vehicle_name','$new_icd_code','$new_icd_out_datetime','$new_factory_code','$new_factory_ea_datetime','$new_factory_ed_datetime','$new_icd_in_datetime','$account_id','$current_date',1,'-')";
				...
				{
					echo "\nMATCH::UPDATING ICD_OUT TO->> ICD_IN_TIME..".$i;
					//$update_flag = true;
					$update_sno = $sno[$j];
		...
		{		
			if($input_icd_in_datetime[$i]=='0000-00-00 00:00:00')
			{
				echo "\nINSERTING NEW ENTRIES..".$i;
				$query_insert = "INSERT INTO icd_input(vehicle_name,icd_code,icd_out_datetime,factory_code,factory_ea_datetime,factory_ed_datetime,icd_in_datetime,account_id,create_date,status,remark) values('$input_vehicle_name[$i]','$icd_code_global','$input_icd_out_datetime[$i]','$input_factory_code[$i]','$input_factory_ea_datetime[$i]','$input_factory_ed_datetime[$i]','$input_icd_in_datetime[$i]','$account_id','$current_date',1,'1')";
				//echo "<br>QueryInsert=".$query_insert;
				$result1 = mysql_query($query_insert, $DbConnection);						
			}
		}
