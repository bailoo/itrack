...
{
	//$startdate='2014-04-23 00:00:00';
	//$enddate='2014-04-23 23:59:59';
		...
		{			
			...
			{      
				...
				{					
					...
					//############# ON THE WAY TO ICD ####################
					$entered_station_halt2 = false;

					if( ($factory_in_once) && ($AddEntryinrReport_halt==false) && (!$entered_station_halt2) )	//IF JUST WENT OUTSIDE OF THE FACTORY
					{
						for($k=0;$k<sizeof($factory_coord_db);$k++)
						{
							...
							if($customer_code_input == $factory_code_db[$k])
							{
								...
								if( ($lat_g!="") && ($lng_g!="") )
								{
									...
									{
										//echo "\nVehicle entered again in Factory ,Datetime=".$datetime;  
										//######################
						...
						if((!$entered_station_halt2) && ($factory_in_time!="") && ($factory_in_once) && (!$factory_out_once))
						{
							...
							//echo "\nDiff=".$diff_factory_time." ,FIN=".$factory_in_time." DT=".$datetime;	
							if($diff_factory_time > 120)
							{
								$status_res = "On the way to ICD";
								//$factory_out_time = $datetime;
							
								//echo "\nDEPT_FACTORY=".$depature_time_halt;
								$factory_out_time = $depature_time_halt;
								//$factory_in_flag = false;							
								$factory_out_once = true;
								//echo "\nON THE WAY TO ICD-2: FactoryOutTime=".$factory_out_time;
							}
						}
					}
					...
					if(($AddEntryinrReport_halt) && ($factory_in_flag==false) && (!$factory_out_once))		//##### IF HALT OCCURED
					{										
	...
	{
		$current_lat = $current_lat_tmp;
		$current_lng = $current_lng_tmp;
	}	
	...
	if( (trim($factory_in_time)=="") && (trim($factory_out_time)==""))
	{
		$status_res = "On the way to Factory";
	}
	else if((trim($factory_in_time)!="") && (trim($factory_out_time)==""))
	{
		//echo "\nAtFactory";
		$status_res = "At Factory";
	}
