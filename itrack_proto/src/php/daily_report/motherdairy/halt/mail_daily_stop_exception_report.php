...
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    ...
                    "vehicle_grouping.account_id=230 AND vehicle.status=1 AND vehicle_assignment.status=1 AND vehicle_grouping.status=1";              
...
$date1 = $startdate;
$date2 = $enddate;
$user_interval = "60";   //SIXTY MINUTES
...
function write_report($vserial, $vid, $vname, $user_interval)
{
  ...
  {
		$inc_serial=$i+1;
		$filename_title = 'VTS_DAILY_STOP_EXCEPTION_REPORT_MOTHERDAIRY'.$previous_date."_".$inc_serial;
  ...
  */
  ...
  $subject = 'VTS_STOP_EXCEPTION_REPORT_MOTHERDAIRY'.$previous_date;
  $message = 'VTS_STOP_EXCETION_MOTHERDAIRY'.$previous_date; 
