...
include_once($abspath."/daily_report/motherdairy/kanpur/get_master_detail.php");
//include_once($abspath."/daily_report/motherdairy/test_delhi_routetype/get_master_detail.php");
...
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    "vehicle.vehicle_id = vehicle_assignment.vehicle_id AND vehicle_assignment.vehicle_id = vehicle_grouping.vehicle_id AND ".
                    "vehicle_grouping.account_id='$account_id' AND vehicle.status=1 AND vehicle_assignment.status=1 AND vehicle_grouping.status=1 and vehicle.vehicle_id IN(2241,2478,2483,2605)";					
*/					
/*			
	$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
	"vehicle.vehicle_id = vehicle_assignment.vehicle_id AND vehicle_assignment.vehicle_id = vehicle_grouping.vehicle_id AND ".
	"vehicle_grouping.account_id='$account_id' AND vehicle.status=1 AND vehicle_assignment.status=1 AND vehicle_grouping.status=1 limit 6";   
*/
 //and vehicle.vehicle_id IN(2602,2615)           
...
//$current_date = date('Y-m-d', strtotime($date .' -2 day')); // LAST 4 DAYS : -2, -3, -4, -5
			...
			//$current_date = date('Y-m-d', strtotime($date .' -1 day'));		//COMMENT IT

///////////////////////////////////////////
$startdate = $current_date." 03:00:00";      // MORNING TIME: 3AM TO 7:30PM
$enddate = $current_date." 23:30:00";
//$enddate = $current_date." 18:00:00"; 
///////////////////////////////////////////

$date1 = $startdate;
$date2 = $enddate;
$user_interval = "1";   //ONE MINUTE
...
$route_type_input = array();
$relative_route_type_input = array();
//echo "\nbefore";
//######## GET MASTER DETAIL ################
//$account_id="231";
$shift_time = "ZPMM";
...
//$startdate = $min_date_mor;
//$enddate = $max_date_mor;  
...
/*			
$query2 = "SELECT DISTINCT station_id,type,customer_no,station_name,station_coord,distance_variable,google_location FROM station WHERE ".
            "user_account_id='$account_id' AND customer_no IN(".$customer_input_string.") AND type=0 AND status=1";			
//echo "\nQ2=".$query2;*/
...
//if($vehicle_id_final!="")
{
	//$inc_serial=$i+1;
	$inc_serial = rand();
	$filename_title = 'BETA_VTS_HALT_REPORT_MORNING_(MOTHER_KANPUR)_'.$current_date."_".$inc_serial."#mor";
	...
	$worksheet2->write($r, 21, "RouteType", $text_format);
	...
	for($p=0;$p<sizeof($csv_string_halt_arr);$p++)
	{
		...
		if($csv_string_halt_arr[$p]!="")
		{
			...
			for($i=0;$i<sizeof($sheet2_row_tmp);$i++)
			{
				...
				$route_type_temp[] = $sheet2_data_tmp[19];				
	...
	{				
		...
		$route_type[$n] = $route_type_temp[$i];
		...
		for($j=1;$j<100;$j++)
		{
			...
			if( $ji_sum < sizeof($vname_temp))
			{
				...
				if( (trim($vname_temp[$ji_sum]) == trim($vname_temp[$i])) && (trim($station_no_temp[$ji_sum]) == trim($station_no_temp[$i])) && (trim($type_str_temp[$ji_sum]) == trim($type_str_temp[$i])) && (trim($route_no_temp[$ji_sum]) == trim($route_no_temp[$i])) )
				{
	...
	{						
		...
		{	
			...
			if($type_str[$i]!="Plant")
			{
				...
				$relative_tmp_string = binary_plant_search($customer_at_the_rateA[0], $relative_customer_input, $relative_plant_input, $relative_transporter_input, $relative_route_input, $relative_route_type_input);				
				...
				$relative_routes_type = $relative_tmp_string_arr[3];					
			...
			if($substr_count == 0)
			{											
				$csv_string_halt_final = $csv_string_halt_final.$vname[$i].','.$sno[$i].','.$station_no[$i].','.$type_str[$i].','.$relative_routes.','.$report_shift[$i].','.$arrivale_time[$i].','.$depature_time[$i].','.$schedule_in_time[$i].','.$time_delay[$i].','.$hrs_min[$i].','.$report_date1[$i].','.$report_time1[$i].','.$report_date2[$i].','.$report_time2[$i].','.$transporter_name_master[$i].','.$relative_transporters.','.$relative_plants.','.$km[$i].','.$relative_routes_type;
				$substr_count =1;  
			}
			else
			{
			...
			$route_type_prev = $relative_routes_type;
	...
	for($q=0;$q<sizeof($sheet2_row);$q++)
	{
		...
		if($sheet2_data[0]!=$prevVehicle)
		{
			...
			if( (trim($vehicle_out_plant) == trim($vehicle_in_plant)) && ($vehicle_out_plant!="" && $plant_out!="" && $vehicle_in_plant!="" && $plant_in!=""))
			{
		...
		if($EntryCnt>0)
		{
			...
			{
				if( (trim($sheet2_data[0]) == trim($prev_vehicles_arr[$k])) && (trim($sheet2_data[2]) == trim($prev_customers_arr[$k])) )
				{
		...
		else if ( (trim($sheet2_data[15])!= trim($sheet2_data[16])) && (trim($sheet2_data[3])!="Plant") )				//TRANSOPORTER NOT MATCHED(15 AND 16-LIGHT BLUE)
		{								
			//echo"BLUE\n";
			...
			for($i=0;$i<sizeof($transporter_arr);$i++)
			{
				for($j=0;$j<sizeof($transporter_arr1);$j++)
				{
					...
					if(trim($transporter_arr1[$j])==trim($transporter_arr[$i]))
					{
						//echo "Matched\n";
    ...
    {    
		...
		if(!$found)
		{
			...
			for($m2=0;$m2<sizeof($relative_customer_input);$m2++)
			{
				...
				if( (trim($customer_no[$m]) == trim($relative_customer_input[$m2])) || (trim($customer_at_the_rate[0]) == trim($relative_customer_input[$m2])) )
				{		  
					$route_i = $relative_route_input[$m2];
					//$route_type_i = $relative_route_type_input[$m2];
			...
			$worksheet2->write($r, 1, "", $text_format);
			//$worksheet2->write($r, 2, "", $text_format);
			$worksheet2->write($r, 2, $customer_at_the_rate2[0], $text_red_format);
	...
	for($x = 1; $x < sizeof($customer_arr); $x++) 
	{
		...
		$tmp_customer_arr = trim($customer_arr[$x]);
		...
		while($done == false)
		{
			$customer_tmp1 = trim($customer_arr[$z]);
			...
			if (trim($customer_tmp1_B[0]) > trim($customer_tmp1_A[0]))
			{
				$vehicle_arr[$z + 1] = $vehicle_arr[$z];
				$customer_arr[$z + 1] = trim($customer_arr[$z]);
	...
	{	
		...
		//###### check @
		$pos_c2 = strpos($customer_arr[$i], "@");
		if($pos_c2 !== false)
		{
			//echo "\nNegative Found";
			$customer_tmp1_i = explode("@", $customer_arr[$i]);
		}
		else
		{
			$customer_tmp1_i[0] = $customer_arr[$i];
		}
		//##########
		
		for($j=$i+1;j<sizeof($customer_arr);$j++)
		{
			//####### Treat @ customer as normal
			$pos_c1 = strpos($customer_arr[$j], "@");
			if($pos_c1 !== false)
			{
				//echo "\nNegative Found";
				$customer_tmp1_j = explode("@", $customer_arr[$j]);
			}
			else
			{
				$customer_tmp1_j[0] = $customer_arr[$j];
			}							
			//###################################
			
			//if($customer_arr[$j]!=$customer_arr[$i])
			if(trim($customer_tmp1_j[0])!= trim($customer_tmp1_i[0]))
			{
				break;
			}
...
*/		
	...
	{
		$tmp_customer_i = trim($customer_i[$x]);
		...
		while($done == false)
		{
			$customer_tmp1 = trim($customer_i[$z]);

			if (trim($customer_tmp1) > trim($tmp_customer_i))
			{
				$customer_i[$z + 1] = trim($customer_i[$z]);							
	...
	for($i=0;$i<sizeof($customer_arr);$i++)						//#### GENERATED IN REPORT
	{
		if( ($type_arr[$i]!="Plant") && (($color_arr[$i] == "white") || ($color_arr[$i] == "pink")) )
		{
			...
			{
				//echo "\nNegative Found";
				$customer_normal = explode("@", $customer_arr[$i]);
			}
			
			$customer_arr2[] = $customer_normal[0];
	...
	for($i=0;$i<sizeof($customer_arr2);$i++)						//#### GENERATED IN REPORT
	{
		...
		if( (trim($customer_arr2[$i]) == trim($customer_prev)) && (trim($vehicle_arr2[$i])!=trim($vehicle_prev)) )
		{
	...
	//$to = 'Vijay.Singh@motherdairy.com,teetu.kumar@motherdairy.com,call1.center@motherdairy.com,ashish.sawhney@motherdairy.com,Rahul.tiwari@motherdairy.com,Shiv.sharma@motherdairy.com,Logistic.Ppg@motherdairy.com,ashish@iembsys.co.in,jitendra3.kumar@motherdairy.com';
	//$to = 'teetu.kumar@motherdairy.com';
	$to = 'Logistics.kanpur@motherdairy.com,Shiv.sharma@motherdairy.com,Vijay.singh@motherdairy.com,Teetu.kumar@motherdairy.com';
	$subject = 'BETA::VTS_HALT_REPORT_MORNING_(MOTHER_KANPUR)_'.$current_date;
	$message = 'BETA::VTS_HALT_REPORT_MORNING_(MOTHER_KANPUR)_'.$current_date."<br><br><font color=red size=1>*** This is an automatically generated email by the system on specified time, please do not reply ***</font>"; 
