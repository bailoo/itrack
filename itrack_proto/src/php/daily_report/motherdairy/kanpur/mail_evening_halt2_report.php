...
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    "vehicle.vehicle_id = vehicle_assignment.vehicle_id AND vehicle_assignment.vehicle_id = vehicle_grouping.vehicle_id AND ".
                    "vehicle_grouping.account_id='$account_id' AND vehicle.status=1 AND vehicle_assignment.status=1 AND vehicle_grouping.status=1 and vehicle.vehicle_id IN(2619)";
*/
/*
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    "vehicle.vehicle_id = vehicle_assignment.vehicle_id AND vehicle_assignment.vehicle_id = vehicle_grouping.vehicle_id AND ".
...
//////////////////////////////////////////
$startdate = $previous_date." 15:00:00";               //TIME 3PM TO 7:30AM
//$enddate = $current_date." 07:30:00"; 
$enddate = $current_date." 09:00:00";		//TIME 3PM TO 09:00AM
/////////////////////////////////////////
...
//$query2 = "SELECT DISTINCT station_id,type,customer_no,station_name,station_coord,distance_variable,google_location FROM station WHERE ".
//"user_account_id='$account_id' AND (customer_no IN(".$customer_input_string.") OR ".$customer_input_string2.") AND type=0 AND status=1 limit 5";
...
//if($vehicle_id_final!="")
{
	//$inc_serial=$i+1;
	$inc_serial = rand();
	$filename_title = 'BETA_VTS_HALT_REPORT_EVENING_(MOTHER_KANPUR)_'.$previous_date."_".$inc_serial."#ev1";
	...
	for($q=0;$q<sizeof($sheet2_row);$q++)
	{
		...
		if( (trim($sheet2_data[3]) == "Customer") && ($plant_trip_flag) )
		{
			...
			//$route_type_str = $route_type_str.$sheet2_data[19]."/";
		...
		if( (trim($sheet2_data[3]) == "Plant") && ($plant_trip_flag) )
		{
			if(trim($vehicle_out_plant) == trim($sheet2_data[0]))
			{
			...
			{					
				if($m==6 || $m==7)	//ARRIVAL, DEPARTURE DATE AND TIME
				{
					...
					if($datetime_tmp[0]!="-" && $datetime_tmp[1]!="-" && $datetime_tmp[0]!="" && $datetime_tmp[1]!="")
					{
						...
						if(($m==7) && ($current_date == $datetime_tmp[0]) && (strtotime($tmp_date)>strtotime($tmp_date1)) && ($tmpclr==0) && (trim($sheet2_data[3])=="Plant"))
						{
							//echo"Test3\n";
							$tmpclr=1;
						}
    ...
    //####### IN DATABASE(PLOTTED) MAY OR MAY NOT IN MASTER : NOT VISITED			//REDS
    ...
    //####### IN MASTER NOT IN DATABASE : NOT VISITED					//BLUES
...
*/	
	...
	{				
		if( (trim($customer_i[$i]) == trim($customer_prev)) && ($route_id[$i]!=$route_prev) )
		{
	...
	$subject = 'BETA::VTS_HALT_REPORT_EVENING_(MOTHER_KANPUR)_'.$previous_date;
	$message = 'BETA::VTS_HALT_REPORT_EVENING_(MOTHER_KANPUR)_'.$previous_date."<br><br><font color=red size=1>*** This is an automatically generated email by the system on specified time, please do not reply ***</font>";; 
