...
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    "vehicle.vehicle_id = vehicle_assignment.vehicle_id AND vehicle_assignment.vehicle_id = vehicle_grouping.vehicle_id AND ".
                    "vehicle_grouping.account_id='$account_id' AND vehicle.status=1 AND vehicle_assignment.status=1 AND vehicle_grouping.status=1 and vehicle.vehicle_id IN(1328)";					
*/
/*
$query_assignment = "SELECT DISTINCT vehicle.vehicle_id,vehicle.vehicle_name FROM vehicle,vehicle_assignment,vehicle_grouping WHERE ".
                    "vehicle.vehicle_id = vehicle_assignment.vehicle_id AND vehicle_assignment.vehicle_id = vehicle_grouping.vehicle_id AND ".
...
$startdate = $previous_date." 15:00:00";               //TIME 3PM TO 12PM
...
//if($vehicle_id_final!="")
{
	//$inc_serial=$i+1;
	$inc_serial = rand();
	$filename_title = 'BETA_VTS_HALT_REPORT_EVENING2_(MOTHER_KANPUR)_'.$previous_date."_".$inc_serial."#ev2";
...
*/	
	...
	{				
		if( (trim($customer_i[$i]) == trim($customer_prev)) && (trim($route_id[$i])!=trim($route_prev)) )
		{
	...
	$subject = 'BETA::VTS_HALT_REPORT_EVENING2_(MOTHER_KANPUR)_'.$previous_date;
	$message = 'BETA::VTS_HALT_REPORT_EVENING2_(MOTHER_KANPUR)_'.$previous_date."<br><br><font color=red size=1>*** This is an automatically generated email by the system on specified time, please do not reply ***</font>";; 
