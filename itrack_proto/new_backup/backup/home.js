...
function show_geofence()
{
    ...
    {	
		...
		{	
			...
			var i;	
			var bounds_global = new GLatLngBounds();	
			var point_global;																				
			...
			{	
				...
				//alert("geo_name="+geo_name);
				opts = 
				{ 
				"icon": icon,
				"clickable": true,
				"title": "letter A",					
				"labelText": "<font color=green size=2><b>"+geo_name+"</b></font>",					
				...
				for(var j=0;j<geo_points2.length;j++)
				{
					...
					if(j == lastpt)
					{
						...
						pts[j] = new GLatLng(parseFloat(points[0]),parseFloat(points[1]));
						polygon[p] = new GPolygon(pts, "#f33f00", 5, 1, "#ff0000", 0.2);
						//alert("pts="+polygon[p]);
