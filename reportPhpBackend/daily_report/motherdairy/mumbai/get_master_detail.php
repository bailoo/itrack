			...
			if($file_ext[0] == "2")
			{
				...
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
						...
						for ($c=0; $c < $num; $c++) {
							...
							{
								$ScheduleInTime2[] = $data[$c];
							}
							else if($c==3)
							{
								$ScheduleOutTime2[] = $data[$c];
							}   
						}
					}
					fclose($handle);
					...
					while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) 
					{
						...
						*/
						...
						if($row > 2)
						{
							...
							$customer_name_input[] = $data[9];
							$customer_input[] = $data[10];
					...
					while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) 
					{
						...
						*/					
						...
						{												
							...
							$relative_customer_name_input[] = $data[4];
							echo "\nEVE7::r=".$row." ,data[3]=".$data[3]." ,data[4]=".$data[4]."\n";
						} 					
					}
					fclose($handle);
					//echo "\nsizeof(route_input)=".sizeof($route_input);
			...
			if( ($file_ext[0] == "8") && ($shift_time=="ZPMM") )			//###### MORNING FILE
			{
				...
				if (($handle = fopen($path, "r")) !== FALSE) {
					...
					while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
						...
						{										
							/*$relative_customer_input[] = $data[0];
							$relative_customer_name_input[] = $data[1];							
							$relative_plant_input[] = $data[2];
							$relative_transporter_input[] = $data[3];
							$relative_route_input[] = $data[4];*/
