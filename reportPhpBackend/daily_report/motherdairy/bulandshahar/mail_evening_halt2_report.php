...
*/
//////////////////////////////////////////
$startdate = $previous_date." 15:00:00";               
$enddate = $current_date." 03:00:00";		       //TIME 3PM TO 03 AM
...
//######## GET MASTER DETAIL ################
$shift_time = "ZPME";
//echo "\nAC=".$account_id." ,ShiftTime=".$shift_time;
...
//if($vehicle_id_final!="")
{
	//$inc_serial=$i+1;
	$inc_serial = rand();
	$filename_title = 'BETA_VTS_HALT_REPORT_EVENING_(MOTHER_BULANDSHAHAR)_'.$previous_date."_".$inc_serial;
	...
	//######################################################
	$worksheet2->write($r, 12, "Halt Duration (Hr:min:sec)", $text_format);		//16
	$worksheet2->write($r, 13, "ReportDate1", $text_format);					//17
	$worksheet2->write($r, 14, "ReportTime1", $text_format);					//18
	$worksheet2->write($r, 15, "ReportDate2", $text_format);					//19
	$worksheet2->write($r, 16, "ReportTime2", $text_format);					//20
        ...
        for($p=0;$p<sizeof($csv_string_halt_arr);$p++)
	{
		...
		if($csv_string_halt_arr[$p]!="")
		{
			...
			for($i=0;$i<sizeof($sheet2_row_tmp);$i++)
			{
				...
				//###############################################
				//echo "##PDelay=".$sheet2_data_tmp[12]." ,".$sheet2_data_tmp[13];
				...
				$km_temp[] = $sheet2_data_tmp[15];
				//$customer_name_temp[] = $sheet2_data_tmp[19];	
	...
	{						
		...
		{	
			...
			if($substr_count == 0)
			{											
				$csv_string_halt_final = $csv_string_halt_final.$vname[$i].','.$sno[$i].','.$station_no[$i].','.$type_str[$i].','.$route_no[$i].','.$report_shift[$i].','.$arrivale_time[$i].','.$depature_time[$i].','.$schedule_in_time[$i].','.$time_delay[$i].','.$hrs_min[$i].','.$report_date1[$i].','.$report_time1[$i].','.$report_date2[$i].','.$report_time2[$i].','.$km[$i];
				$substr_count =1;  
			}
			else
			{
    ...
    $worksheet2->write($r, 15, "", $text_format);
    $worksheet2->write($r, 16, "", $text_format);
    /*$worksheet2->write($r, 17, "Plant", $text_red_format);
    $worksheet2->write($r, 18, "", $text_format);
    $worksheet2->write($r, 19, "", $text_format);*/
    $r++;         
    ...
    {    
		...
		if(!$found)
		{
			...
			$worksheet2->write($r, 15, "", $text_format);
                        $worksheet2->write($r, 16, "", $text_format);
			/*$worksheet2->write($r, 16, $transporter_i, $text_red_format);
			$worksheet2->write($r, 17, $plant_i, $text_red_format);
			$worksheet2->write($r, 18, "", $text_format);*/
    ...
    {    
		...
		{			
			...
			$worksheet2->write($r, 15, "", $text_format);
                        $worksheet2->write($r, 16, "", $text_format);
			/*$worksheet2->write($r, 16, $relative_transporter_input[$m], $text_blue_format);
			$worksheet2->write($r, 17, $relative_plant_input[$m], $text_blue_format);*/
			$r++;
		}
    }	
	...
	$to = 'Ashok.tripathi@motherdairy.com,Chetan.sharma@motherdairy.com,dinesh.arya@motherdairy.com,mccbulandshahr@gmail.com,MohdMalik@motherdairy.com';

	$subject = 'BETA::VTS_HALT_REPORT_EVENING_(MOTHER_BULANDSHAHAR)_'.$previous_date;
	$message = 'BETA::VTS_HALT_REPORT_EVENING_(MOTHER_BULANDSHAHAR)_'.$previous_date."<br><br><font color=red size=1>*** This is an automatically generated email by the system on specified time, please do not reply to this email***</font>"; 
