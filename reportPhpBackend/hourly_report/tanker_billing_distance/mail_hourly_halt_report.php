...
$read_file_path = $abspath . "/gps_report/" . $user_name . "/upload/".$date;
$write_file_path = $abspath . "/gps_report/" . $user_name . "/download/".$date;
echo "\nSent_RootPath=" . $write_file_path;

$evening_sent_file_path1 = $write_file_path . "/V6_HOURLY_MAIL_VTS_HALT_REPORT_EVENING_MOTHER_TANKER_ROUTE.xlsx";
...
if (!$MAIN_DEBUG) {
    if ($shift_ev1) {
        $result = exec("lsof +d $write_file_path | grep -c -i V6_HOURLY_MAIL_VTS_HALT_REPORT_EVENING_MOTHER_TANKER_ROUTE.xlsx");
        if ($result == "1") {
            $shift_ev1 = false;
        }
    }
    ...
    if ($shift_ev2) {
        $result = exec("lsof +d $write_file_path | grep -c -i V6_HOURLY_MAIL_VTS_HALT_REPORT_EVENING_MOTHER_TANKER_FOCAL.xlsx");
        if ($result == "1") {
            $shift_ev2 = false;
        }
    }
    ...
    if ($shift_mor) {
        $result = exec("lsof +d $write_file_path | grep -c -i V6_HOURLY_MAIL_VTS_HALT_REPORT_MORNING_MOTHER_TANKER.xlsx");
        if ($result == "1") {
            $shift_mor = false;
        }
    }
