...
function get_halt_xml_data($vehicle_serial, $vid, $vname, $startdate, $enddate, $user_interval, $report_shift)
{
	...
	{	
		$xml_current = $back_dir_current."/current_data/xml_data/".$userdates[$i]."/".$vehicle_serial.".xml";
		...
		{
			$xml_file = $back_dir_sorted."/".$userdates[$i]."/".$vehicle_serial.".xml";
			$CurrentFile = 0;
		}		
		...
		{			
			...
			{      
				...
				while(!feof($xml))          // WHILE LINE != NULL
				{
					...
					//echo "\nXML_DATE=".$xml_date." ,DataValid=".$DataValid." ,vehicle_serial=".$vehicle_serial." ,StartDate=".$startdate." ,EndDate=".$enddate;
