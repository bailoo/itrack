...
$startdate = $previous_date." 21:00:00";      // MORNING : ZPMM……..9pm to 9am(including next date)
$enddate = $current_date." 09:00:00"; 
/////////////////////////////////////////
...
//if($vehicle_id_final!="")
{
	//$inc_serial=$i+1;
	$inc_serial = rand();
	$filename_title = 'BETA_VTS_HALT_REPORT_MORNING_(MOTHER_MUMBAI)_'.$previous_date."_".$inc_serial;
	...
	for($p=0;$p<sizeof($csv_string_halt_arr);$p++)
	{
		...
		if($csv_string_halt_arr[$p]!="")
		{
			...
			for($i=0;$i<sizeof($sheet2_row_tmp);$i++)
			{
				...
				//echo "\n".$sheet2_data_tmp[20].",".$sheet2_data_tmp[21];
	...
	{						
		...
		{	
			...
			if($substr_count == 0)
			{											
				$csv_string_halt_final = $csv_string_halt_final.$vname[$i].','.$sno[$i].','.$station_no[$i].','.$type_str[$i].','.$route_no[$i].','.$report_shift[$i].','.$arrivale_time[$i].','.$depature_time[$i].','.$schedule_in_time[$i].','.$time_delay[$i].','.$pschedule_in[$i].','.$pschedule_out[$i].','.$ptime_delay1[$i].','.$ptime_delay2[$i].','.$hrs_min[$i].','.$report_date1[$i].','.$report_time1[$i].','.$report_date2[$i].','.$report_time2[$i].','.$transporter_name_master[$i].','.$relative_transporters.','.$relative_plants.','.$km[$i].",".$relative_customer_name;
				$substr_count =1;  
			}
			else
			{
	...
	$to = 'Logistics.Vashi@motherdairy.com, vivek.ghadge@motherdairy.com, Anand.Arondekar@motherdairy.com, Vijay.Singh@motherdairy.com, ashish@iembsys.co.in';

	$subject = 'BETA::VTS_HALT_REPORT_MORNING_(MOTHER_MUMBAI)_'.$previous_date;
	$message = 'BETA::VTS_HALT_REPORT_MORNING_(MOTHER_MUMBAI)_'.$previous_date."<br><br><font color=red size=1>*** This is an automatically generated email by the system on specified time, please do not reply to this email***</font>"; 
