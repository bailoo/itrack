...
function get_halt_xml_data($vehicle_serial, $vid, $vname, $startdate, $enddate, $user_interval, $report_shift)
{
	...
	//$back_dir_sorted = "/mnt/volume4";
        ...
        $LastSortedDate = getLastSortedDate($vehicle_serial, $datefrom, $dateto);
		...
		//{			
			...
			//{      
				...
				//while(!feof($xml))          // WHILE LINE != NULL
                                for ($x = 0; $x < sizeof($xml_date_sel); $x++) {
                                
                                        $xml_date = $xml_date_sel[$x];
                                        $datetime = $xml_date;
                                        $lat = $lat_sel[$x];
                                        $lng = $lng_sel[$x];
                                        $speed = $speed_sel[$x];					
					...
					if($xml_date!=null)
					{
						...
						if(($xml_date >= $startdate && $xml_date <= $enddate))
						{
							//echo "\nIN DATE";
					...
					{
						//echo "\nELSEIF HALT1, datetime=".$datetime;
						$arrivale_time = $datetime_ref;
						$starttime = strtotime($datetime_ref);										  
						$AddEntryinrReport=true;
					}*/
