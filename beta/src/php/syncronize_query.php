<?php 
	...
	$sync = $_POST['sync'];
	/*$vendor_group_id="0002";
	$login_id_1="UP003";
	$password_1=md5('sales1');	
	$imei_1= "359462041789701";		
	$sync = "orderitems";*/
	$flag=1;
	$in_data=0;
	if($sync=="expense")
							...
							else
							{
								...
								if($num_rows>0)
								{
									...
									$query="SELECT max(vendor_code) as vendor_code FROM vendor_detail WHERE sales_person_id=$sales_id".
											" vendor_id IN(SELECT vendor_id FROM vendor WHERE status=1) AND status=1";				
									$result=mysql_query($query,$DbConnection);
									$row=mysql_fetch_row($result);
									$vendor_code=$row[0];
									if($vendor_code=="")
								...
								else
								{
									...
									if($result)
									{
										...
										$query="SELECT vendor_id from vendor WHERE create_date='$date' AND status=1";	
										...
										if($result)
										{
											...
											$query="INSERT INTO uom_vendor_assignment(uom_id,vendor_id,create_id,create_date,status)VALUES('$uom_ids','$row[0]','1','$date','1')";
											$result=mysql_query($query,$DbConnection);
											if($result)
											{
