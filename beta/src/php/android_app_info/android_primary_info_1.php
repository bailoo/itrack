<?php
	...
	$android_group_id = trim($_POST['group_id']);
	$android_user_id =trim($_POST['user_id']);
	$android_password=md5(trim($_POST['password']));
	$sync = $_POST['sync'];
	...
	if($sync=="login")
	{
		...
		if($num_rows>0)
		{
			...
			echo "success,".$type.",".$row1->name.",".$row[1];						
		}
		else
		{
			echo "failure";
