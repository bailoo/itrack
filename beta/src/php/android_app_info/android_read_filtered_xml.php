...
//////// FUNCTION READ TRACK XML STARTS //////////////
  ...
  { 
	...
	function read_track_xml_person($xml_path, &$vehicleserial, &$lat, &$lng, &$datetime, &$vehiclename, &$vehicletype)
  { 
	global $va,$vb,$vc,$vd,$ve,$vg,$vh,$vv,$vw,$vx,$vy,$vz,$vaa,$vab;
	global $old_xml_date;
	new_xml_variables();
...
// FUNCTION READ TRIP DATA OPENS
  function read_tripdata_xml($xml_path, &$datetime, &$vehicleserial, &$vehiclename, &$lat, &$lng)
  {    
    $fexist =1;
    $fix_tmp = 1;
    $final_xml = $xml_path;
        ...
        if( ($line[0] == '<') && ($line[strlen($line)-2] == '>') && ($fix_tmp==1) && ($format == 1) && ($DataValid == 1))
        {
          //  echo "<br>IN format1";             
           ...
           // echo "test1".'<BR>';
            ...
            //echo "test2".'<BR>';
            $status = preg_match('/vname="[^"]+/', $line, $vehiclename_tmp);
            //echo "status=".$status."<br>";
            //if($status==0)
            //{
              //continue;
