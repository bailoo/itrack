...
?>
	<fieldset class="manage_fieldset_invoice">
		...
		<!--<table style="border:thin;" class="manage_interface" cellspacing="2" cellpadding="2" rules="all" align="center" width=100%>-->
			...
			if( $user_type=="plant_raw_milk"){
				$csv_string = $csv_string."SNO,FIRST CREATE DATE,LAST EDIT BY TRANSP,PLANT FIRST GATE ENTRY EDIT,LORRY NO,VEHICLE NO,TANKER TYPE,DOCKET NO,EMAIL,TRANSPORTER MOBILE,QTY,FAT(%),SNF(%),FAT(KG),SNF(KG),MANUAL MILK AGE(Hrs),DISPATCH TIME,TARGET TIME,DRIVER NAME,DRIVER MOBILE,USERNAME(USERID),STATUS, PLANT,LECINO,CHILLING PLANT,MATERIAL,UNLOAD ESTIMATED TIME,GATE ENTRY,POSTING DATE/TIME,FAT%(FT),SNF%(FT),Qty(FT),Temp.(FT),Acidity(FT),MBRT-min(FT),RM(FT),BR(FT),Protien%(FT),Sodium(FT),Testing Status,FAT%(RT),SNF%(RT),ADULTRATION,OtherADULTRATION,CLOSE TIME\n";
			}
			else if( $user_type=="raw_milk"){
				$csv_string = $csv_string."SNO,FIRST CREATE DATE,LAST EDIT BY TRANSP,PLANT FIRST GATE ENTRY EDIT,LORRY NO,VEHICLE NO,TANKER TYPE,DOCKET NO,EMAIL,TRANSPORTER MOBILE,QTY,FAT(%),SNF(%),FAT(KG),SNF(KG),MANUAL MILK AGE(Hrs),DISPATCH TIME,TARGET TIME,DRIVER NAME,DRIVER MOBILE,USERNAME(USERID),STATUS, PLANT,LECINO,CHILLING PLANT,MATERIAL,APPROVED TIME,UNLOAD ESTIMATED TIME,GATE ENTRY,POSTING DATE/TIME,FAT%(FT),SNF%(FT),Qty(FT),Temp.(FT),Acidity(FT),MBRT-min(FT),RM(FT),BR(FT),Protien%(FT),Sodium(FT),Testing Status,FAT%(RT),SNF%(RT),ADULTRATION,OtherADULTRATION \n";
			}
			else{
				$csv_string = $csv_string."SNO,FIRST CREATE DATE,LAST EDIT BY TRANSP,PLANT FIRST GATE ENTRY EDIT,LORRY NO,VEHICLE NO,TANKER TYPE,DOCKET NO,EMAIL,TRANSPORTER MOBILE,QTY,FAT(%),SNF(%),FAT(KG),SNF(KG),MANUAL MILK AGE(Hrs),DISPATCH TIME,TARGET TIME,DRIVER NAME,DRIVER MOBILE,USERNAME(USERID),STATUS, PLANT,LECINO,CHILLING PLANT,MATERIAL,APPROVED TIME,UNLOAD ESTIMATED TIME,GATE ENTRY,POSTING DATE/TIME,FAT%(FT),SNF%(FT),Qty(FT),Temp.(FT),Acidity(FT),MBRT-min(FT),RM(FT),BR(FT),Protien%(FT),Sodium(FT),Testing Status,FAT%(RT),SNF%(RT),ADULTRATION,OtherADULTRATION,CLOSE TIME\n";
