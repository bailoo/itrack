...
?>
	<fieldset class="manage_fieldset_invoice">
		...
		<!--<table style="border:thin;" class="manage_interface" cellspacing="2" cellpadding="2" rules="all" align="center" width=100%>-->
			...
			<th>SNO<div>SNO</div></th><th>FIRST CREATED<br>(General)<div>FIRST CREATED<br>(General)</div></th><th>TRACK EDITED<br>(Transp.Last/PlantFirstClose)<div>TRACK EDITED<br>(Transp.Last/PlantFirstClose)</div></th><th>L.R. No./Challan No<div>L.R. No./Challan No</div></th><th>VEHICLE NO<div>VEHICLE NO</div></th><th>TANKER TYPE<div>TANKER TYPE</div></th><th>DOCKET NO<div>DOCKET NO</div></th><th>EMAIL/SUPPLIER NAME<div>EMAIL/SUPPLIER NAME</div></th><th>TRANSPORTER MOBILE<div>TRANSPORTER MOBILE</div></th><th>QTY(KG)<div>QTY(KG)</div></th>
			...
			echo "<tr class='header' style='background-color:rgb(189, 104, 104);' rules='all'>
			<th>SNO<div>SNO</div></th><th>FIRST CREATED<br>(General)<div>FIRST CREATED<br>(General)</div></th><th>TRACK EDITED<br>(Transp.Last/PlantFirstClose)<div>TRACK EDITED<br>(Transp.Last/PlantFirstClose)</div></th><th style='background-color:yellow'>L.R. No./Challan No<div style='background-color:yellow'>L.R. No./Challan No</div></th><th style='background-color:orange'>VEHICLE NO<div style='background-color:orange'>VEHICLE NO</div></th><th>TANKER TYPE<div>TANKER TYPE</div></th><th>DOCKET NO<div>DOCKET NO</div></th><th>EMAIL/SUPPLIER NAME<div>EMAIL/SUPPLIER NAME</div></th><th>TRANSPORTER MOBILE<div>TRANSPORTER MOBILE</div></th><th style='background-color:blue'>QTY(KG)<div style='background-color:blue'>QTY(KG)</div></th>
			...
			if( $user_type=="plant_raw_milk"){
				$csv_string = $csv_string."SNO,FIRST CREATE DATE,LAST EDIT BY TRANSP,PLANT FIRST GATE ENTRY EDIT,LORRY NO/CHALLAN NO,VEHICLE NO,TANKER TYPE,DOCKET NO,EMAIL/SUPPLIER,TRANSPORTER MOBILE,QTY,FAT(%),SNF(%),FAT(KG),SNF(KG),MANUAL MILK AGE(Hrs),DISPATCH TIME,TARGET TIME,DRIVER NAME,DRIVER MOBILE,USERNAME(USERID),STATUS, PLANT,LECINO,CHILLING PLANT,MATERIAL,UNLOAD ESTIMATED TIME,GATE ENTRY,POSTING DATE/TIME,FAT%(FT),SNF%(FT),Qty(FT),Temp.(FT),Acidity(FT),MBRT-min(FT),RM(FT),BR(FT),Protien%(FT),Sodium(FT),Testing Status,FAT%(RT),SNF%(RT),ADULTRATION,OtherADULTRATION,CLOSE TIME\n";
			}
			else if( $user_type=="raw_milk"){
				$csv_string = $csv_string."SNO,FIRST CREATE DATE,LAST EDIT BY TRANSP,PLANT FIRST GATE ENTRY EDIT,LORRY NO/CHALLAN NO,VEHICLE NO,TANKER TYPE,DOCKET NO,EMAIL/SUPPLIER,TRANSPORTER MOBILE,QTY,FAT(%),SNF(%),FAT(KG),SNF(KG),MANUAL MILK AGE(Hrs),DISPATCH TIME,TARGET TIME,DRIVER NAME,DRIVER MOBILE,USERNAME(USERID),STATUS, PLANT,LECINO,CHILLING PLANT,MATERIAL,APPROVED TIME,UNLOAD ESTIMATED TIME,GATE ENTRY,POSTING DATE/TIME,FAT%(FT),SNF%(FT),Qty(FT),Temp.(FT),Acidity(FT),MBRT-min(FT),RM(FT),BR(FT),Protien%(FT),Sodium(FT),Testing Status,FAT%(RT),SNF%(RT),ADULTRATION,OtherADULTRATION \n";
			}
			else{
				$csv_string = $csv_string."SNO,FIRST CREATE DATE,LAST EDIT BY TRANSP,PLANT FIRST GATE ENTRY EDIT,LORRY NO/CHALLAN NO,VEHICLE NO,TANKER TYPE,DOCKET NO,EMAIL/SUPPLIER,TRANSPORTER MOBILE,QTY,FAT(%),SNF(%),FAT(KG),SNF(KG),MANUAL MILK AGE(Hrs),DISPATCH TIME,TARGET TIME,DRIVER NAME,DRIVER MOBILE,USERNAME(USERID),STATUS, PLANT,LECINO,CHILLING PLANT,MATERIAL,APPROVED TIME,UNLOAD ESTIMATED TIME,GATE ENTRY,POSTING DATE/TIME,FAT%(FT),SNF%(FT),Qty(FT),Temp.(FT),Acidity(FT),MBRT-min(FT),RM(FT),BR(FT),Protien%(FT),Sodium(FT),Testing Status,FAT%(RT),SNF%(RT),ADULTRATION,OtherADULTRATION,CLOSE TIME\n";
            ...
            foreach($result as $row_select)
			{
				...
				";
					...
					echo '</td>'
					;
					echo "<td>".$row_select['chilling_plant']."&nbsp;[".$row_select['station_name']."]</td>";
				...
				echo"<input TYPE=\"hidden\" VALUE=\"$row_select[lorry_no]\" NAME=\"temp[$i][LORRY NO/CHALLAN NO]\">";
				...
				echo"<input TYPE=\"hidden\" VALUE=\"$row_select[email]\" NAME=\"temp[$i][EMAIL/SUPPLIER]\">";
