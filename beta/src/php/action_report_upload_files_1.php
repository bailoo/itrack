...
if($account_id!="")
{
    ...
    $filePathToAccount=$pathToRoot."/s3/itrack/gps_report";
    $dirNameAccount=$account_id;
        
    
    $dirCreateStatus=dirExistsS3($filePathToAccount, $dirNameAccount);
    if($dirCreateStatus=='')
    {
        makeDirS3($filePathToAccount, $dirNameAccount);
    }
        
    $filePathToDownload=$pathToRoot."/s3/itrack/gps_report/".$account_id;
    $dirNameDownload="download";
    $dirCreateStatus=dirExistsS3($filePathToDownload, $dirNameDownload);
    if($dirCreateStatus=='')
    {
        makeDirS3($filePathToDownload, $dirNameDownload);
    }
    
    $filePathToMaster=$pathToRoot."/s3/itrack/gps_report/".$account_id;
    $dirNameMaster="master";
    $dirCreateStatus=dirExistsS3($filePathToMaster, $dirNameMaster);
    if($dirCreateStatus=='')
    {
        makeDirS3($filePathToMaster, $dirNameMaster);
    }
    
    $filePathToUpload=$pathToRoot."/s3/itrack/gps_report/".$account_id;
    $dirNameUpload="upload";
    $dirCreateStatus=dirExistsS3($filePathToUpload, $dirNameUpload);
    if($dirCreateStatus=='')
    {
        makeDirS3($filePathToUpload, $dirNameUpload);
    }
    ...
    {	
        ...
        $j=0;	
        $store_files=opendir($filePathToMaster."/".$dirNameMaster);
    ...
    else if($upload_type=="get_report")
    {
        $random_number=rand();
        $format_ids_1=explode(":",$format_ids);
        $filePathToDate=$pathToRoot."/s3/itrack/gps_report/".$account_id."/upload";
        $dirNameDate=$date1[0];
        $dirCreateStatus=dirExistsS3($filePathToDate, $dirNameDate);
        if($dirCreateStatus=='')
        {
            makeDirS3($filePathToDate, $dirNameDate);
        }                   		
        $j=0;
        $file_ids_arr=explode(",",$format_ids_1[1]);
    }
    ...
    {		
        ...
        $tmp_upload_file=$pathToRoot."/s3/itrack/tmp_upload_file/".$final_file_name;
        if($upload_type=="master")
        {
            $final_dest_file=$pathToRoot."/s3/itrack/gps_report/".$account_id."/master/".$final_file_name;	
            if($format_ids_1[0]=="#3")
            {						
                delete_files($post_formatid_arr,$post_filename_arr,$format_ids_1[0],$pathToRoot);
...
function delete_file_with_error_message($f_code,$original_file_name,$tmp_upload_file,$tmp_val)
{
	...
	if($flag==0)
	{
            ...
            {		
                delete_files($delete_post_formatid_arr[$i],$delete_post_filename_arr[$i],$delete_format_ids_1[$i],$pathToRoot);	
            }
	echo'<table border="0" align="center">
                <tr>
                    <td>
	...
	function upload_files($final_dest_file,$format_ids_cnd,$upload_type,$sub_files_ids)
	{
            ...
            {
                $overWrite=1;
                $uploadStatus=uploadFile($final_dest_file, $_FILES['file_'.$format_ids_cnd."_".$sub_files_ids]['tmp_name'], $overWrite);
	...
	function delete_files($post_formatid_arr,$post_filename_arr,$format_ids_cnd,$pathToRoot)
	{
            $account_id=$_SESSION['account_id'];
            if(isset($post_formatid_arr))
            {
                for($fd=0;$fd<sizeof($post_formatid_arr);$fd++) //////delete previous master file with same id coming from post method
                {
                    ...
                    {
                        $delFilePath=$pathToRoot."/gps_report/".$account_id."/master";
                        $delFileName=$post_filename_arr[$fd];
                        delFile($delFilePath,$delFileName);
