			  ...
			  $map .= "
						...
						geocoder.geocode({'latLng': latlng}, function(results, status) 
						{
							...
							if (status == google.maps.GeocoderStatus.OK) 
							{
								...
								if(results) 
								{
								   ...
								   '<table><tr><td class=live_td_css1>StationName &nbsp;:&nbsp;<input type=text name=station_name id=station_name  size=10></td><td class=live_td_css1>CustomerNo&nbsp;:&nbsp;<input type=text name=customer_no id=customer_no size=10><input type=hidden name=landmark_point id=landmark_point value=\''+latlng+'\'></td>'+			 					
				...
				{ 
					...
					for(var i=0;i<latarr.length;i++)
					{
						...
						google.maps.event.addListener
						(
							marker, 'click', infoCallbackTrack(lat_tmp,lng_tmp,dateTime,vSerialLocal,vNameLocal,vNumberLocal,speedLocal,marker,ioStrLocal,dTravelLocal,feature_id_map)
						);						
				...
				{ 
					...
					for(var i=0;i<latarr.length;i++)
					{
						...
						google.maps.event.addListener
						(
							marker, 'click', infoCallbackPersonTrack(lat_tmp,lng_tmp,dateTime,vSerialLocal,vNameLocal,vNumberLocal,speedLocal,marker,dTravelLocal)
						);						
				...
				{  
					...
					for(var i=0;i<latarr.length;i++)
					{
						...
						google.maps.event.addListener
						(						
							marker, 'click', infoCallbackPerson(lat_tmp,lng_tmp,dateTime,vSerialLocal,vNameLocal,vNumberLocal,marker)
						);					
					}
				...
				{  
					...
					for(var i=0;i<latarr.length;i++)
					{
						...
						google.maps.event.addListener
						(						
							marker, 'click', infoCallback(lat_tmp,lng_tmp,dateTime,vSerialLocal,vNameLocal,vNumberLocal,speedLocal,ioStrLocal,marker,dMaxSpeedLocal,lHaltSpeedLocal)
						);					
					}
