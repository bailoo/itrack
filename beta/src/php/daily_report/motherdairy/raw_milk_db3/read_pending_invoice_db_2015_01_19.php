...
function read_pending_invoice_db()
{
   ...
   $query = "SELECT DISTINCT sno,lorry_no,vehicle_no,email,mobile,qty_kg,fat_percentage,snf_percentage,fat_kg,snf_kg,milk_age,docket_no,dispatch_time,target_time,validity_time,plant_acceptance_time,plant,chilling_plant,driver_name,driver_mobile,unload_estimated_time,unload_accept_time,parent_account_id,create_date,invoice_status,close_type,close_time,system_time FROM `invoice_mdrm` WHERE ( (parent_account_id = '$account_id') OR (parent_account_id IN (SELECT account_id FROM account_detail WHERE account_admin_id='$admin_id')) AND (plant!='' AND chilling_plant!='') AND ( (create_date BETWEEN '$sdatetime_open' and '$cdatetime') OR (edit_date BETWEEN '$sdatetime' and '$cdatetime') OR (invoice_status='1') OR (close_time between '$pdatetime' AND '$cdatetime'))) AND dispatch_time > '2015-01-01 00:00:00' AND status=1 AND vehicle_no='HR696468' GROUP BY (vehicle_no) ORDER BY vehicle_no DESC";
   
 /*$query = "SELECT DISTINCT sno,lorry_no,vehicle_no,email,mobile,qty_kg,fat_percentage,snf_percentage,fat_kg,snf_kg,milk_age,docket_no,dispatch_time,target_time,validity_time,plant_acceptance_time,plant,chilling_plant,driver_name,driver_mobile,unload_estimated_time,unload_accept_time,parent_account_id,create_date,invoice_status,close_type,close_time,system_time FROM `invoice_mdrm` WHERE ( (parent_account_id = '$account_id') OR (parent_account_id IN (SELECT account_id FROM account_detail WHERE account_admin_id='$admin_id')) AND (plant!='' AND chilling_plant!='') AND ( (create_date BETWEEN '$pdatetime' and '$cdatetime') OR (invoice_status='1') OR (close_time between '$pdatetime' AND '$cdatetime'))) AND dispatch_time > '2015-01-01 00:00:00' AND status=1 AND vehicle_no IN('MP06HC1335') ORDER BY vehicle_no DESC";*/
    ...
    while($row = mysql_fetch_object($result))
    {
		...
		{			
			...
			if($numrows4>0)
			{
				...
				$query_name = "SELECT name FROM account_detail WHERE account_id='$paccount_id'";
				//echo "\nQname=".$query_name;
				$result_name = mysql_query($query_name,$DbConnection);
				$row_name = mysql_fetch_object($result_name);
				$transporter_id[] = $paccount_id;
			...
			$query_admin = "SELECT account_detail.admin_id FROM account_detail,account WHERE account.account_id='$paccount_id' AND account.status=1 AND account.account_id=account_detail.account_id";
			//echo "<br>P_ID=".$paccount_id;
			...
			get_child_transporter($paccount_id,$admin_id);
			
			//echo "<br>SizeTPT_ChildAcc=".sizeof($transporter_child_account[$paccount_id]);
			for($t=0;$t<sizeof($transporter_child_account[$paccount_id]);$t++)
			{
				$tpt_acc = $transporter_child_account[$paccount_id][$t];
				...
				{
					$child_chilling_plant[$paccount_id][] = $row_t->customer_no;
				}
			}
			
			//echo "<br>SizeCP=".sizeof($child_chilling_plant[$paccount_id]);
			for($c=0;$c<sizeof($child_chilling_plant[$paccount_id]);$c++)
			{
				$cno = $child_chilling_plant[$paccount_id][$c];
				...
				{					
					//echo "<br>CP Stored";
					$chilling_plant_no[$paccount_id][] = $row5->customer_no;
					$chilling_plant_coord_total[$paccount_id][] = $row5->station_coord;
					$chilling_plant_distvar_total[$paccount_id][] = $row5->distance_variable;
				}
			}			
		}
    }
