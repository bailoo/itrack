...
function get_master_detail($account_id, $shift_time)
{
	...
	//$dir = "/var/www/html/vts/beta/src/php/daily_report/motherdairy/test_delhi_routetype/gps_report/".$account_id."/master";
	echo "\nDIR=".$dir;
	$dh = opendir($dir);
				...
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) 
				{
					...
					*/
					...
					if($row > 2)
					{
						$shift_input[] = $data[4];
						$route_input[] = $data[6];
						$route_type_input[] = $data[7];	//ADDED
						$vehicle_input[] = $data[8];
				...
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) 
				{
					...
					*/					
					...
					{												
						...
						$relative_route_type_input[] = $data[4];
						//echo "\nEv:Data[4]=".$data[4];
