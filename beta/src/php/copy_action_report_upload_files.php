...
if($account_id=="322" || $account_id=="1100" || $account_id=="1115")
{
	...
	function upload_files($final_dest_file,$format_ids_cnd,$upload_type,$sub_files_ids)
	{
		...
		{					
			...
			echo'<table border="0" align="center" class="menu">
				...
				if((($upload_type=="master" && $format_ids_cnd=="#4") || ($upload_type=="master" && $format_ids_cnd=="#9")) && ($account_id=='231' || $account_id=='322' || $account_id=='723' || $account_id=='568' || $account_id=='2' || $account_id=='1100'  || $account_id=='1115'  || $account_id=='718' || $account_id=="232"))
				{
					$shift_time = "ZPME";
					get_master_detail($account_id, $shift_time); 
				}
				if((($upload_type=="master" && $format_ids_cnd=="#5") || ($upload_type=="master" && $format_ids_cnd=="#10")) && ($account_id=='231' || $account_id=='322' || $account_id=='723' || $account_id=='568' || $account_id=='2' || $account_id=='1100' || $account_id=='1115'  || $account_id=='718' || $account_id=="232"))
				{
					//echo "in if <br>";
					$shift_time = "ZPMM";
					get_master_detail($account_id, $shift_time);
