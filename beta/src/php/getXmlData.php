   ...
   // $st_obj = gpsParser($st_results);
	...
	{		
		foreach($st_results as $item) 
		{
			...
			//echo "switchDatetime=".$switchDatetime."<br>";
			if (($switchDatetime >= $startDate && $switchDatetime <= $endDate) && ($switchDatetime != "-") && ($switchDatetime != null) && $DataValid==1) 
			{
...
function getLastRecord($vSerial,$sortBy,$parameterizeData)
{
	...
	//exit();
			...
			if ($DataValid) 
			{
				...
				$dataObject->dayMaxSpeedLR[]='-';
				$dataObject->dayMaxSpeedTimeLR[]='-';
				$dataObject->lastHaltTimeLR[]='-';	
