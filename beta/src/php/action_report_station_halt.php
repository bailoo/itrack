...
<?php
    ...
    echo'<form method = "post" target="_blank">';
		...
		{
			$csv_string = $csv_string."SNo,Vehicle Name,Customer Number,Date,Enter Time,Leave Time,Duration (Hrs.min),Shift\n";
		}
	...
	else if($account_id == "723")
	{
		$user_id = "ice_cream";
	}
	else if($account_id == "322")
	{
		$user_id = "mumbai";
	}
	...
	for($i=0;$i<sizeof($aryRange_1);$i++)
	{
		...
		{
			//$file_path="daily_report/bvm/report_log/tmp/".$aryRange_1[$i];
			//$file_path="daily_report/motherdairy/test_halt2/excel_reports/".$aryRange_1[$i];
			$file_path="daily_report/motherdairy/".$user_id."/excel_reports/".$aryRange_1[$i];
		}
		...
		foreach ($objPHPExcel->setActiveSheetIndex(0)->getRowIterator() as $row) 
		{
			...
			if($k>1)
			{
				...
				foreach ($cellIterator as $cell) 
				{
					...
					if($ci==7)
					{	
						$excel_in_date1=$objPHPExcel->getActiveSheet()->getCell($tmp_val)->getValue();
						$excel_in_date = PHPExcel_Style_NumberFormat::toFormattedString($excel_in_date1, 'YYYY-MM-DD');						
					}
					if($ci==8)
					{
						$excel_in_time1=$objPHPExcel->getActiveSheet()->getCell($tmp_val)->getValue();
						$excel_in_time = PHPExcel_Style_NumberFormat::toFormattedString($excel_in_time1, 'hh:mm:ss');						
					}					
					if($ci==10)
					{
						$excel_out_time1=$objPHPExcel->getActiveSheet()->getCell($tmp_val)->getValue();
						$excel_out_time = PHPExcel_Style_NumberFormat::toFormattedString($excel_out_time1, 'hh:mm:ss');
					}
					if($ci==13)
					{
						$excel_halt_duration1=$objPHPExcel->getActiveSheet()->getCell($tmp_val)->getValue();
						$excel_halt_duration = PHPExcel_Style_NumberFormat::toFormattedString($excel_halt_duration1, 'hh:mm:ss');
					}
					if($ci==6)
					{
				...
				$excel_tmp_str=$excel_tmp_str.$excel_vehicle_name.",".$excel_station_no.",".$excel_in_date.",".$excel_in_time.",".$excel_out_time.",".$excel_halt_duration.",".$excel_shift.",";
				//echo "excel_tmp_str=".$excel_tmp_str."<br>";
				$excel_tmp_str=  substr($excel_tmp_str, 0, -1);
				$excel_data_array[]=$excel_tmp_str;								
			}
		}
		//echo "out excel file<br>";
		for($vi=0;$vi<sizeof($vstr_arr);$vi++)
		{
			...
			for($cui=0;$cui<sizeof($station_str_tmp1);$cui++)
			{
				for($exi=0;$exi<sizeof($excel_data_array);$exi++)
				{
					...
					// echo "customer=".$station_str_tmp1[$cui]." data=".$excel_data_array1[1]." vname=".$vname_post." data=".$excel_data_array1[0]."<br>";
					if(($station_str_tmp1[$cui]==$excel_data_array1[1]) && ($vname_post==$excel_data_array1[0]))                                           
					{ 
						$flag=1;
						$final_data_array[]=$scnt."@".$vname_post.'@'.$excel_data_array1[1]."@-@".$excel_data_array1[2]."@".$excel_data_array1[3]."@".$excel_data_array1[4]."@".$excel_data_array1[5]."@".$excel_data_array1[6];
						$scnt++;   
						//echo "geoloction=".$excel_data_array1[1]." in time=".$excel_data_array1[3]." outtime=".$excel_data_array1[6]." duration=".$excel_data_array1[7]."<br>";
					} 
				}                                
			}
		}
	}	
	function get_csv_data($file_path_1,$vstr_arr,$station_str_tmp1)
	{
		//echo "in csv<br>";
		global $flag;
		global $final_data_array;
	 ...
	 if($flag==0)
	  ...
	  {
		//echo "final_size=".sizeof($final_data_array)."<br>";
		if(sizeof($final_data_array)>=23)
		{
