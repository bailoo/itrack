<?php    
 ...
 // $place_name_arr=$_SESSION['place_name_arr1'];
  $startdate = $_POST['startdate'];
	...
	$time_intervalA = $_POST['time_interval'];
	//$time_intervala = $time_interval/60;
	//echo $time_intervalA."<br>";
	if($time_intervalA >=0 && $time_intervalA <60){
		$time_intervalA=$time_intervalA." Seconds";
	}
	else if($time_intervalA >= 60 && $time_intervalA <3600 ){
		$time_intervalA=$time_intervalA/60 ." Minutes";
	}
	else{
		$time_intervalA=$time_intervalA/3600 ." Hours";
	}
  ...
  //print_r($location);
	...
	read_xml_imei2($xmltowrite, &$vehicleserialA, &$lat, &$lng, &$noloc ,&$alt ,&$datetime, &$vehiclename, &$vehicletype, &$speed, &$cumdist, &$io1, &$io2, &$io3, &$io4, &$io5, &$io6, &$io7, &$io8);
					...
					<table border="0" width = "100%" cellspacing="2" cellpadding="0" align="center">
						<tr>
							<td height=10 class="text" align="center" width="79%">
								...
								<font color=green>
									'.$vehiclename[0].'</font>&nbsp;<font color=red>('.$vehicleserialA[0].')
								</font> 
								<br> 
								<font color="#23238E">
									(Date Time:
						...
						echo'<center><div style="font-size:10px;"><strong>(&nbsp;Time Interval &nbsp;:&nbsp;'.$time_intervalA;
								...
								<table align="center" border="0" width="100%" bordercolor="#689FFF"  cellspacing="6" cellpadding="8">
									...
									{ 
										if($i==0)
										{
											if($report_type=='Person')
											{
												$title1="Person :- ".$vehiclename[$i]." (".$vehicleserialA[$i]." )  DateTime :".$startdate." - ".$enddate." )";
											}
											else
											{
												$title1="Vehicle :- ".$vehiclename[$i]." (".$vehicleserialA[$i]." )  DateTime :".$startdate." - ".$enddate." )";;
											}
