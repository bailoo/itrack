...
var input = (document.getElementById('pac-input'));		
  ...
  google.maps.event.addListener(map_canvas, 'bounds_changed', function() {
    ...
    return array.hasOwnProperty(prop) && /^0$|^[1-9]\d*$/.test(prop) && prop <= 4294967294; // 2^32 - 2
				...
				function infoCallbackTrack(lat,lng,dateTimeArr,vSerial,vName,speed,marker,CID) 
				{					
					return function() 
					{						
						var contentString='';
							...
							if (status == google.maps.GeocoderStatus.OK) 
							{
								...
								if(results) 
								{
								   ...
								   '<tr>'+
									...
									'</tr>'+
									'<tr>'+                                                    
									'<td class=\"live_td_css1\">CellID</td>'+
									'<td>:</td>'+
									'<td class=\"live_td_css2\">'+CID+'</td>'+
									'</tr>';
                                ...
                                function calculate_distance(lat1, lat2, lon1, lon2) 
    {
    	...
    	return distance;
				...
				function setMultipleMarker(map,latarr, lngarr,datetimearr,vSerial,vName,speed,CID)
				{ 
                                //alert('MAP='+map+' ,latarr='+latarr);
					deleteOverlays();
					...
					for(var i=0;i<latarr.length;i++)
					{
                       ...
                       //var dMobileNoLocal=dMobileNoArr[i];
						...
						google.maps.event.addListener
						(
							marker,'click', infoCallbackTrack(lat_tmp,lng_tmp,dateTime,vSerialLocal,vNameLocal,speedLocal,marker,cellID)
						);						
	...
	*
	*/
	function addMultipleMarker($map_id, $latarr,$lngarr,$datetimearr,$vehicle_serial_arr,$vehicle_name_arr,$speed_arr,$cellIDArr)
	{
		//print_r($latarr);
		//print_r($lngarr);
		...
		if(is_array($latarr) && is_array($lngarr))
		{
			...
			$speedJs=json_encode($speed_arr);
                        //echo "speedJs=".$speedJs."<br>";
			//$ioStr=json_encode($io_str_last);			
			//$dTravelJs=json_encode($d_travel_str); 
			$cellIDJS=json_encode($cellIDArr);			
			//echo "in if<br>";
			$marker .= "setMultipleMarker({$map_id},{$latJs},{$lngJS},{$dateTimeJs},{$vSerialJs},{$vNameJs},{$speedJs},{$cellIDJS}".")";
		}		
		$marker .= "</script>";
		return $marker;
	}
