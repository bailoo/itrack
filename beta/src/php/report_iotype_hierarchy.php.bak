...
//////////////// FUNCTIONS ////////////////////////////////////////////		
		...
		{	
			...
			{								  
				...
				<td class=\'text\'>&nbsp;<A HREF="#" style="text-decoration:none;" onclick="main_vehicle_information('.$vehicle_id.')"><font color="'.$vcolor2.'">'.$vehicle_name.'</font>&nbsp;'; if($type_tag==1) {echo'('.$option_name.')';} echo'</font></A></td>';
			}
			else
			{
				//echo "<br>2";
		...
		function get_all_vehicle_radio_interval($local_account_id,$div_option_values)
		{
			//echo "<br>accnode=".$AccountNode." ,div_option_values".$div_option_values;
		  global $root;
		  global $vehicleid;
			...
			//$user_name = GetUserName($AccountNode,$div_option_values);	
		  if($div_option_values == "all")
			PrintAllVehicleRadioInterval($root, $local_account_id);
		}
...
function get_all_vehicle_radio($local_account_id,$div_option_values)
{
  ...
  if($div_option_values == "all")
		...
		if($root->data->AccountID==$local_account_id)
		{
		  $td_cnt =0;
			...
			{			    
				...
				$iovalueandtypearr =  $root->data->DeviceIOTypeValue[$j];
				//echo "io value=".$iovalueandtypearr."<br>";
				$tmp_iotype_str="";
				...
				if($vehicle_id!=null)
				{
					...
					if($i>=$vehicle_cnt)
					{
						...
						if(file_exists($xml_current))
						{
							...
							{
								$vehicleid_or_imei_arr[$color][$vehicle_name]=$vehicle_imei.$tmp_iotype_str;
							}
						}
						else
						{ 
	...
	function PrintAllVehicleRadioInterval($root, $local_account_id)
	{
	  global $vehicleid;
		global $vehicle_cnt;
		global $td_cnt;
		...
		if($root->data->AccountID==$local_account_id)
		{
		  $td_cnt =0;
			...
			$color = $vcolor2;common_display_vehicle_radio($vehicle_name_arr[$color],$vehicleid_or_imei_arr[$color],$color,'');
	  $color = $vcolor3; common_display_vehicle_radio($vehicle_name_arr[$color],$vehicleid_or_imei_arr[$color],$color,'');
		$ChildCount=$root->ChildCnt;
		for($i=0;$i<$ChildCount;$i++)
		{ 
		 PrintAllVehicleRadioInterval($root->child[$i],$local_account_id);
		}
	}
...
{	
  ...
  //echo "<br>xml_file=". $xml_file;	
    ...
    //echo "<br>exists";
    //echo "td_cnt=".$td_cnt."<br>";
	   echo'<td align="left">&nbsp;<INPUT TYPE="checkbox"  name="vehicleserial[]" VALUE="'.$person_imei.'"></td>
		 <td class=\'text\'>&nbsp;<A HREF="#" style="text-decoration:none;" onclick="main_vehicle_information('.$person_id.')"><font color="darkgreen">'.$person_name.'</font>&nbsp;'; echo'</A></td>';
  }
  else
  {
     echo'<td align="left">&nbsp;<INPUT TYPE="checkbox"  name="vehicleserial[]" VALUE="'.$person_imei.'"></td>
		 <td class=\'text\'>&nbsp;<A HREF="#" style="text-decoration:none;" onclick="main_vehicle_information('.$person_id.')">'.$person_name.'&nbsp;';if($type_tag==1) {echo'('.$option_name.')';} echo'</A></td>';
  }
