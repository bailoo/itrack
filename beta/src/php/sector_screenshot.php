...
//INPUT NEEDED VEHICLE_ID AND PERSON NAME
$vehicle_id1 = $_GET['vid'];

$query_v = "SELECT * device_imei_no FROM vehicle_assignment WHERE vehicle_id ='$vehicle_id1' AND status=1";
$result_v = mysql_query($query_v,$DbConnection);
$row_v = mysql_fetch_object($result_v);
$vserial = $row_v->device_imei_no; 
...
*/		
    ...
    $size_vserial = sizeof($vserial);
		...
		report_title("Sector Report",$param1,$param2);
    ...
    //echo "<br>xml_path=".$xml_path;
		read_sector_data_xml($xml_path, &$imei, &$vname, &$lat, &$lng, &$arr_time, &$dep_time, &$duration, &$route_name, &$sector_name);
    ...
    for($i=0;$i<sizeof($imei);$i++)
      ...
      if(($i===0) || (($i>0) && ($imei[$i-1] != $imei[$i])) )
      {
        ...
        $j++;
        $sno = 1;
        $title="Sector Report : ".$vname[$i]." &nbsp;<font color=red>(".$imei[$i].")</font>-Interval:".$user_interval." mins";
        $vname1[$j][$k] = $vname[$i];
        $imei1[$j][$k] = $imei[$i];
      ...
      echo'<td class="text" align="left"><font color="red"><strong>'.$route_name[$i].'</strong></font></td>';
      echo'<td class="text" align="left"><font color="green"><strong>'.$sector_name[$i].'</strong></font></td>';						
			echo'</tr>';							
			...
			$route_name1[$j][$k] = $route_name[$i];
			$sector_name1[$j][$k] = $sector_name[$i];
    ...
    for($x=0;$x<=$j;$x++)
        ...
        for($y=0;$y<=$no_of_data[$x];$y++)
        {
          ...
          if($y==0)
          {
          	$title="Sector Report : ".$vname1[$x][$y]." (".$imei1[$x][$y].")-Interval:".$user_interval." mins";
          	...
          	$csv_string = $csv_string."SNo,Arrival Time,Departure Time,Halt Duration (Hrs.min), Route Name, Sector Name\n";
          ...
          $csv_string = $csv_string.$sno_1.','.$pdf_arrival_time.','.$pdf_depature_time.','.$pdf_halt_duration.','.$pdf_route_name.','.$pdf_sector_name."\n";
          ////////////////////////////////////         	
        }	//inner for	
		} // outer for		
		...
		if($vsize==0)
		{						
			print"<center><FONT color=\"Red\" size=2><strong>Sorry! No Sector Record Found</strong></font></center>";
			//echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL=HaltReport.php\">";
			echo'<br><br>';
		}	
		else
		{
      echo'<input TYPE="hidden" VALUE="sector" NAME="csv_type">';
