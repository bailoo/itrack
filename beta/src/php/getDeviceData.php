...
function openCassandraConnection()
{
    global $s_server_host, $s_server_username, $s_server_password, $s_server_keyspace, $i_server_port;

	echo "\n".$s_server_host.", ".$s_server_username.", ".$s_server_password.", ".$s_server_keyspace.", ".$i_server_port;
    $o_cassandra = new Cassandra();	
    $o_cassandra->connect($s_server_host, $s_server_username, $s_server_password, $s_server_keyspace, $i_server_port);
    return $o_cassandra;
}

function closeCassandraConnection($o_cassandra)
{
    $o_cassandra->close();
}

function readDataBetweenDatetime($vSerial, $startDate, $endDate, $userInterval, $requiredData, $sortBy, $type, $parameterizeData, $firstDataFlag,$o_cassandra,&$dataObject) {
    $imei = $vSerial;
    $date = $startDate;
    $HH = '23';
...
function readDataDateWise($vSerial, $dateToData,  $requiredData, $sortBy, $parameterizeData, $o_cassandra, &$dataObject) {
    $imei = $vSerial;
    $HH = '23';
   if ($sortBy == "h")
   {
   ...
   else if($sortBy == "g")
   {
    ...
    foreach($st_results as $item) {
        ...
        if ($parameterizeData->cellId!= null) 
        {
            $dataObject->cellIdData[] = $cell_id;
        }
...
function getLastDataBetweenDates($vSerial,$startDate,$endDate,$xmlFromDate,$xmlToDate,$sortBy,$type,$parameterizeData,$o_cassandra,&$dataObject)
{
   if ($sortBy == "h")
   {
        $deviceTime=TRUE;
...
function getLastSeenRecord($vSerial,$sortBy,$o_cassandra,$parameterizeData)
{
	$imei = $vSerial;
...
function getAllDates($fromDate, $toDate, &$userdates) {
