                        ...
                        if($sortBy=="h")  /// for device datetime
			{
                            if(count($SortedDataObject->deviceDatetime)>0)
                            {
                                ...
                                for($obi=0;$obi<$prevSortedSize;$obi++)
                                {
                                    ...
                                    $flowRate = $SortedDataObject->flowRateData[$obi];
echo "<br>FlowRate=".$flowRate;                              
                                    if($firstdata_flag==0)
                                    {					
                                        $firstdata_flag = 1;
                        ...
                        if($sortBy=="g")  /// for device datetime
			{
                            if(count($SortedDataObject->serverDatetime)>0)
                            {
                                ...
                                for($obi=0;$obi<$sortedSize;$obi++)
                                {
                                    ...
                                    $flowRate = $SortedDataObject->flowRateIOData[$obi]; 					  
                                    if($firstdata_flag==0)
                                    {					
                                        $firstdata_flag = 1;
                                        $interval = (double)$userInterval*60;	     
...
echo '<center>';
  ...
  echo'<br>';
  report_title("FlowRate Report",$date1,$date2);
  ...
  for($i=0;$i<sizeof($imei);$i++)
    ...
    if(($i==0) || (($i>0) && ($imei[$i-1] != $imei[$i])) )
    {
      ...
      $sno = 1;
      $title='FlowRate Report : '.$vname[$i]." &nbsp;<font color=red>(".$imei[$i].")</font>";
      $vname1[$j][$k] = $vname[$i];
      $imei1[$j][$k] = $imei[$i];   
      ...
      <tr>
            ...
            <td class="text" align="left"><b>FlowRate</b></td>								
      </tr>';  								
    }                                                                        		
...
$ftemperature=substr_replace($flowRatethis1, '.', 2, -1);*/
    ...
    //$flowRate1[$j][$k] = round($distance[$i],2); 
    //$flowRate1[$j][$k] = $ftemperature;  
$flowRate1[$j][$k] = $flowRateDisplay[$i];       			    				  					
  ...
  for($x=0;$x<=$j;$x++)
	{								
		$title = $vname1[$x][0]." (".$imei1[$x][0]."): FlowRate Report- From DateTime : ".$date1."-".$date2;
		$csv_string = $csv_string.$title."\n";
    $csv_string = $csv_string."SNo,Start Time,End Time,FlowRate\n";
  ...
  echo'	
    <table align="center">
		<tr>
  		...
  		if(sizeof($imei)==0)
  		{						
  			print"<center><FONT color=\"Red\" size=2><strong>No FlowRate Record Found</strong></font></center>";
  			//echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"1; URL=HaltReport.php\">";
  			echo'<br><br>';
  		}	
  		else
  		{
        echo'<input TYPE="hidden" VALUE="flowRate" NAME="csv_type">';
