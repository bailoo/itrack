				...
				for(i=0;i<imeino1.length;i++)
				{
					if(imeino1[i].checked)
					{
						if(cnt==0)
						{
							...
							obj_unique[value_tmp]=value_tmp;
						}
						else
						{
							var value_tmp=imeino1[i].value;
	...
	*/
	...
	{  
		...
		{		
			...
			{
				alert("Customer did not add.Please add customer");
				return false;
			}
		}
		...
		if(client_type_combo=="1")
		{
			...
			{
				alert("Plant did not add.Please add plant");
				return false;
			}
		}
	}
...
{  
  ...
  for(var i=0;i<landmark_counter;i++)
  {
    ...
    if(search_text == landmark_name_list[i])
    {
      ...
      found_point = landmark_point_list[i];
  ...
  if(found_landmark_name!="")
  {
    //alert("In found_station_name");
   	var lt_1 = found_point.y; 
  	var ln_1 = found_point.x;
...
{
	//map.clearOverlays();
	var point = new GLatLng(parseFloat(lt_1),parseFloat(ln_1));
	var lat = Math.round((lt_1)*100000)/100000;
	var lng = Math.round((ln_1)*100000)/100000;
	var type_name;
	var Icon= new GIcon(station_icon);
	var marker_local = new GMarker(point,Icon);
	//alert("marker_local="+marker_local);
	
	map.addOverlay(marker_local);	
  ...
  //alert("iwform="+iwform);
	...
	var infoTabs = [tab1];
	marker_local.openInfoWindowTabsHtml(infoTabs);	
}	
/// *** SHOW STATION CLOSED
...
{		
	//alert("In function lt="+lt_1+" ,lng="+ln_1+" ,found_landmark_name="+landmark);
	//alert("lnmrk="+landmark);
	//var Icon= new GIcon(landmark_icon);
	//alert('icon='+Icon);
   ...
   /* //// GET VNAME //////////////
    ...
    /////////////////////////////////////////////		
		...
		{																													
			...
			vehiclenumber_arr[len2] = xml_data[k].getAttribute("vnumber");
     ...
     // fuel_arr[len2] = xml_data[k].getAttribute(io);	
		io1_arr[len2]=xml_data[k].getAttribute("io1");
		io2_arr[len2]=xml_data[k].getAttribute("io2");
		io3_arr[len2]=xml_data[k].getAttribute("io3");
		io4_arr[len2]=xml_data[k].getAttribute("io4");
		io5_arr[len2]=xml_data[k].getAttribute("io5");
		io6_arr[len2]=xml_data[k].getAttribute("io6");
		io7_arr[len2]=xml_data[k].getAttribute("io7");
		io8_arr[len2]=xml_data[k].getAttribute("io8");
			/*if(fuel_arr[len2] <30)
			{
				fuel_arr[len2] =0;
			}*/											
			vehicletype_arr[len2] = xml_data[k].getAttribute("vtype");
			datetime_arr[len2] =  xml_data[k].getAttribute("datetime");
			
			if(label_type1!="Person")
			{
        day_max_speed_arr[len2] =  xml_data[k].getAttribute("day_max_speed");
        day_max_speed_time_arr[len2] =  xml_data[k].getAttribute("day_max_speed_time");                         
			  last_halt_time_arr[len2] =  xml_data[k].getAttribute("last_halt_time");
			}
...
function get_xml_location(point)
{
	...
	var distance = calculate_distance(point.y, place_name_temp_param[1], point.x, place_name_temp_param[2]);
 ...
 if (result.Status.code == G_GEO_SUCCESS) // OR !=200
    ...
    if(label_type!="Person")
     ...
     // var content1 = '<table cellpadding=0 cellspacing=0><tr><td><font size=2 color=#000000>Vehicle</font></td><td>&nbsp;:&nbsp;</td><td><font color=blue size=2>'+vehiclename + '</font></td><td></td></tr><tr><td><font size=2 color=#000000>IMEI</font></td><td>&nbsp;:&nbsp;</td><td><font color=blue size=2>'+imei + '</font></td><td></td></tr><tr><td><font size=2 color=#000000>Speed</font></td><td>&nbsp;:&nbsp;</td><td><font color=blue size=2>'+speed+' kmph</font></td></tr><tr><td><font size=2 color=#000000>Date & Time</font></td><td>&nbsp;:&nbsp;</td><td><font color=blue size=2>'+datetime+' '+'&nbsp;&nbsp;</font></td></tr><tr><td style="height:30px;"><font size=2 color=#000000>Distance travelled</font></td><td>&nbsp;:&nbsp;</td><td><font color=blue size=2>'+dist+' km</font></td></tr><tr><td><font size=2 color=#000000>Place</font></td><td>&nbsp;:&nbsp;</td><td><font color=blue size=2>'+place+'</font></td></tr><tr><td colspan=3><font color=blue size=2>( '+point.y+', '+point.x+' )</font></td></tr></table>';
      var content1 = '<table cellpadding=0 cellspacing=0><tr><td '+window_style1+'>Vehicle</td><td>&nbsp;:&nbsp;</td><td '+window_style2+'>'+vehiclename + '</td><td></td></tr><tr><td '+window_style1+'>IMEI</td><td>&nbsp;:&nbsp;</td><td '+window_style2+'>'+imei + '</td><td></td></tr><tr><td '+window_style1+'>Speed</td><td>&nbsp;:&nbsp;</td><td '+window_style2+'>'+speed+' kmph</td></tr><tr><td '+window_style1+'>Date & Time</td><td>&nbsp;:&nbsp;</td><td '+window_style2+'>'+datetime+' '+'&nbsp;&nbsp;</td></tr><tr><td '+window_style1+'>Distance travelled</td><td>&nbsp;:&nbsp;</td><td '+window_style2+'>'+dist+' km</td></tr><tr><td '+window_style1+'>Place</td><td>&nbsp;:&nbsp;</td><td '+window_style2+'>'+place+'</td></tr>'+customer_plant_str+customer_plant_str1+io_str+'<tr><td colspan=3 height="5px"></td></tr><tr><td colspan=3 '+window_style1+'>( '+point.y+','+point.x+' )</td></tr></table>';
      var content2 =         
       '<br><form action="#">'
...
{	
  ...
  //var landmark_point = lat+" ,"+lng;
	...
	var url = "src/php/map_add_station.php?station_name="+station_name+"&customer_no="+customer_no+"&landmark_point="+landmark_point;
  //alert(url);
