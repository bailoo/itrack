<?php
	...
	$row_aai_ai=getAcccountAdminIdAdminId($account_id,$DbConnection)
	$aadminId=$row_aai_ai[1];
	$account_ad_id=$row_aai_ai[0];
	
	$parent_admin_id=getAccountIdByAdminId($account_ad_id,$DbConnection);
	global $user_name;	
	...
	//plant
	$DataPlant = getDetailAllCustomerNoStationNameStation($account_id,$DbConnection);
	...
	//chilling plant	
	$dataChil =getCustomerNoStationNext($account_id,$DbConnection) ;
	...
	*/
	...
	function read_sent_db($tid_p,$sno_p)
	{
		...
		global $DbConnection;
		$dataInv= getDetailAllInvoiceMdrm($tid_p,$sno_p,$DbConnection);
	...
	function read_sent_db_pending($pid)
	{
		...
		$dataInvNext= getDetailAllInvoiceMdrmNext($DbConnection);
		foreach($dataInvNext as $dt)
		{	
			$LRNO[] = $dt['LRNO'];
			$Vehicle[] = $dt['Vehicle'];
			$Transporter[] =  $dt['Transporter'];
