	...
	function SortFile($xml_unsorted, $xml_original_tmp,$conditional_date)
	{
		...
		if($fexist)
		{
			...
			while(!feof($xml))          // WHILE LINE != NULL
			{
				...
				//echo "DataValid=".$DataValid." line=".$line[0]." lineLen=".$line[strlen($line)-2]."<br>";
				...
				{
					$status = preg_match('/'.$va.'="[^" ]+/', $line, $msgtype_tmp);
					if($status==0)
					{
						//continue;
					}
					...
					$status = preg_match('/'.$vq.'="[^" ]+/', $line, $sigstr_tmp);
					if($status==0)
					{
						//continue;
					}					
				...
				{   
					//echo "<br>In Format11";
					$msgtype[$i] = $msgtype_tmp[0].'"';                // Store Name with Value
					$sts[$i] = $sts_tmp[0].'"';                // Store Name with Value
					$datetime[$i] = $datetime_tmp[0].'"';                // Store Name with Value
					$version[$i] = $version_tmp[0].'"';                // Store Name with Value
					$lat[$i] = $lat_tmp[0].'"';
					$lng[$i] = $lng_tmp[0].'"';
					$speed[$i] = $speed_tmp[0].'"';
					$fuel[$i] = $fuel_tmp[0].'"';
			...
			//if($format == 1)
			{
				...
				for($y=0;$y<$i;$y++)
				{
					...
					if($imei_flag==1)
					{
						$linetowrite =  "\n"."<x ".$msgtype[$y]." ".$version[$y]." ".$lat[$y]." ".$lng[$y]." ".$speed[$y]." ".$sts[$y]." ".$datetime[$y]." ".$io1[$y]." ".$io2[$y]." ".$io3[$y]." ".$io4[$y]." ".$io5[$y]." ".$io6[$y]." ".$io7[$y]." ".$io8[$y]." ".$fix[$y]." ".$sigstr[$y]." ".$sup_v[$y]."/>";
					}
					else
					{
						$linetowrite =  "\n"."<x ".$msgtype[$y]." ".$version[$y]." ".$lat[$y]." ".$lng[$y]." ".$speed[$y]." ".$sts[$y]." ".$datetime[$y]." ".$io1[$y]." ".$io2[$y]." ".$io3[$y]." ".$io4[$y]." ".$io5[$y]." ".$io6[$y]." ".$io7[$y]." ".$io8[$y]." ".$fix[$y]." ".$sigstr[$y]." ".$sup_v[$y]."/>";
