<?php
	...
	//EVENING SHIFT
	$query_assigned_ev = "SELECT vehicle_name,route_name_ev,evening_update_time,remark_ev FROM route_assignment2 WHERE user_account_id='$account_id' AND status=1";	
	//echo $query_assigned_ev;
	...
	//MORNING SHIFT	
	$query_assigned_mor = "SELECT vehicle_name,route_name_mor,morning_update_time,remark_mor FROM route_assignment2 WHERE user_account_id='$account_id' AND status=1";	
			...
			<table width='100%'>						
				<tr>
					...
					<table border=1 cellspacing=4 cellpadding=4 class='module_left_menu' rules='all' bordercolor='gray'>";
						...
						<td></td>
						<td colspan=3 align=left><font color=red><strong>Evening Shift</strong></font></td>
						<td colspan=3 align=left><font color=red><strong>Morning Shift</strong></font></td></tr>
	...
	function get_route_detail($account_id, $shift_time)
	{
		...
		//$dir = "/var/www/html/beta/src/php/gps_report/".$account_id."/master";
		$dh = opendir($dir);
	...
	function common_function_for_vehicle($vehicle_imei,$vehicle_id,$vehicle_name,$option_name)
	{
		...
		$route_matched_ev = "";
		$remark_matched_ev = "";
		for($k=0;$k<sizeof($vname_assigned_ev);$k++)
		{
			if(trim($vname_assigned_ev[$k]) == trim($vehicle_name))
			{
				$found_ev = 1;
				$route_matched_ev = $route_assigned_ev[$k];				
				$update_time_ev1 = $update_time_ev[$k];
				$remark_matched_ev = $remark_assigned_ev[$k];
				break;
			}
		}
		...
		global $update_time_mor;			
		global $remark_assigned_mor;

		//$found_ev = 0;
		$route_matched_mor = "";
		$remark_matched_mor = "";
		for($k=0;$k<sizeof($vname_assigned_mor);$k++)
		{
			if(trim($vname_assigned_mor[$k]) == trim($vehicle_name))
			{
				$found_mor = 1;
				$route_matched_mor = $route_assigned_mor[$k];
				$update_time_mor1 = $update_time_mor[$k];
				$remark_matched_mor = $remark_assigned_mor[$k];
				break;
			}
		}
